use {
    ...
    put_account_decoder::validator_info::{
    ...
    put_cli_output::{CliValidatorInfo, CliValidatorInfoVec},
    put_config_program::{config_instruction, get_config_data, ConfigKeys, ConfigState},
...
fn verify_keybase(
    ...
    if let Some(keybase_username) = keybase_username.as_str() {
        let url =
            format!("https://keybase.pub/{keybase_username}/put/validator-{validator_pubkey:?}");
...
fn parse_validator_info(
    ...
    if account.owner != put_config_program::id() {
    ...
    fn validator_info_subcommands(self) -> Self {
        self.subcommand(
            SubCommand::with_name("validator-info")
                .about("Publish/get Validator info on PUT")
                .setting(AppSettings::SubcommandRequiredElseHelp)
                .subcommand(
                    SubCommand::with_name("publish")
                        .about("Publish Validator info on PUT")
                        .arg(
                ...
                .subcommand(
                    SubCommand::with_name("get")
                        .about("Get and parse PUT Validator info")
                        .arg(
                            Arg::with_name("info_pubkey")
                                .index(1)
                                .value_name("PUBKEY")
...
pub fn process_set_validator_info(
    ...
    let all_config = rpc_client.get_program_accounts(&put_config_program::id())?;
