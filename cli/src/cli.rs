use {
    ...
    put_clap_utils::{self, input_parsers::*, keypair::*},
    put_cli_config::ConfigInput,
    put_cli_output::{
        display::println_name_value, CliSignature, CliValidatorsSortOrder, OutputFormat,
    },
    put_remote_wallet::remote_wallet::RemoteWalletManager,
    ...
    put_rpc_client_nonce_utils::blockhash_query::BlockhashQuery,
    put_sdk::{
        clock::{Epoch, Slot},
    ...
    put_vote_program::vote_state::VoteAuthorize,
...
pub enum CliError {
    ...
    #[error("Account {1} has insufficient funds for fee ({0} PUT)")]
    InsufficientFundsForFee(String, Pubkey),
    #[error("Account {1} has insufficient funds for spend ({0} PUT)")]
    InsufficientFundsForSpend(String, Pubkey),
    #[error("Account {2} has insufficient funds for spend ({0} PUT) + fee ({1} PUT)")]
    InsufficientFundsForSpendAndFee(String, String, Pubkey),
    #[error(transparent)]
    InvalidNonce(put_rpc_client_nonce_utils::Error),
...
impl From<put_rpc_client_nonce_utils::Error> for CliError {
    fn from(error: put_rpc_client_nonce_utils::Error) -> Self {
        match error {
            put_rpc_client_nonce_utils::Error::Client(client_error) => {
...
pub fn parse_command(
    ...
    let response = match matches.subcommand() {
        ...
        ("completion", Some(matches)) => {
            ...
            .gen_completions_to("put", shell_choice, &mut stdout());
            std::process::exit(0);
        }
        // Cluster Query Commands
        ...
        ("deploy", Some(_matches)) => clap::Error::with_description(
            "`put deploy` has been replaced with `put program deploy`",
...
pub fn process_command(config: &CliConfig) -> ProcessResult {
    ...
    match &config.command {
        ...
        // Return software version of put-cli and cluster entrypoint node
        CliCommand::Catchup {
            node_pubkey,
            node_json_rpc_url,
            follow,
        ...
        // Request an airdrop from PUT Faucet;
...
mod tests {
    use {
        super::*,
        serde_json::json,
        put_rpc_client::mock_sender_for_cli::SIGNATURE,
        put_rpc_client_api::{
            request::RpcRequest,
            response::{Response, RpcResponseContext},
        },
        put_rpc_client_nonce_utils::blockhash_query,
        put_sdk::{
            pubkey::Pubkey,
            signature::{
    ...
    fn test_generate_unique_signers() {
        ...
        assert_eq!(
            signer_info.index_of(Some(put_sdk::pubkey::new_rand())),
            None
        );
    ...
    fn test_cli_parse_command() {
        ...
        let from_pubkey = Some(put_sdk::pubkey::new_rand());
        let from_str = from_pubkey.unwrap().to_string();
        for (name, program_id) in &[
            ("STAKE", stake::program::id()),
            ("VOTE", put_vote_program::id()),
    ...
    fn test_cli_process_command() {
        ...
        assert_eq!(process_command(&config).unwrap(), "0.00000005 PUT");
        ...
        let new_authorized_pubkey = put_sdk::pubkey::new_rand();
        ...
        let bob_keypair = Keypair::new();
        let bob_pubkey = bob_keypair.pubkey();
        let custodian = put_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = put_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = put_sdk::pubkey::new_rand();
        let source_stake_account_pubkey = put_sdk::pubkey::new_rand();
