...
use {
    ...
    put_cli_output::{parse_sign_only_reply_string, OutputFormat},
    put_faucet::faucet::run_local_faucet,
    put_rpc_client::rpc_client::RpcClient,
    put_rpc_client_nonce_utils::blockhash_query::{self, BlockhashQuery},
    put_sdk::{
...
fn test_transfer() {
    ...
    request_and_confirm_airdrop(&rpc_client, &config, &sender_pubkey, put_to_lamports("5.0"))
        .unwrap();
    // feature comment
    // rent change
    // check_balance!(put_to_lamports(5.0.to_string()), &rpc_client, &sender_pubkey);
    ...
    config.command = CliCommand::Transfer {
        amount: SpendAmount::Some(put_to_lamports("1.0")),
        to: recipient_pubkey,
        from: 0,
        sign_only: false,
    ...
    // rent change
    // check_balance!(
    //     put_to_lamports(4.0.to_string()) - fee_one_sig,
    //     &rpc_client,
    //     &sender_pubkey
    // );
    // check_balance!(put_to_lamports(1.0.to_string()), &rpc_client, &recipient_pubkey);

    // Plain ole transfer, failure due to InsufficientFundsForSpendAndFee
    config.command = CliCommand::Transfer {
        amount: SpendAmount::Some(put_to_lamports("4.0")),
        to: recipient_pubkey,
        from: 0,
        sign_only: false,
        dump_transaction_message: false,
    ...
    //     &sender_pubkey
    // );
    check_balance!(put_to_lamports("1.0"), &rpc_client, &recipient_pubkey);
    ...
    request_and_confirm_airdrop(&rpc_client, &offline, &offline_pubkey, put_to_lamports("1.0"))
        .unwrap();
    check_balance!(put_to_lamports("1.0"), &rpc_client, &offline_pubkey);

    // Offline transfer
    let blockhash = rpc_client.get_latest_blockhash().unwrap();
    offline.command = CliCommand::Transfer {
        amount: SpendAmount::Some(put_to_lamports("0.5")),
        to: recipient_pubkey,
        from: 0,
        sign_only: true,
        dump_transaction_message: false,
    ...
    // rent fee change
    // check_balance!(
    //     put_to_lamports(0.5.to_string()) - fee_one_sig,
    //     &rpc_client,
    //     &offline_pubkey
    // );
    check_balance!(put_to_lamports("1.5"), &rpc_client, &recipient_pubkey);
    ...
    // rent fee change
    // check_balance!(
    //     put_to_lamports(4.0.to_string()) - fee_one_sig - fee_two_sig - minimum_nonce_balance,
    //     &rpc_client,
    //     &sender_pubkey,
    // );

    // Fetch nonce hash
    let nonce_hash = put_rpc_client_nonce_utils::get_account_with_commitment(
        &rpc_client,
        &nonce_account.pubkey(),
        CommitmentConfig::processed(),
    )
    .and_then(|ref a| put_rpc_client_nonce_utils::data_from_account(a))
    .unwrap()
    .blockhash();
    ...
    // rent fee change
    // check_balance!(
    //     put_to_lamports(3.0.to_string()) - 2 * fee_one_sig - fee_two_sig - minimum_nonce_balance,
    //     &rpc_client,
    //     &sender_pubkey,
    // );
    check_balance!(put_to_lamports(&2.5.to_string()), &rpc_client, &recipient_pubkey);
    let new_nonce_hash = put_rpc_client_nonce_utils::get_account_with_commitment(
        &rpc_client,
        &nonce_account.pubkey(),
        CommitmentConfig::processed(),
    )
    ...
    // rent fee change
    // check_balance!(
    //     put_to_lamports(3.0.to_string()) - 3 * fee_one_sig - fee_two_sig - minimum_nonce_balance,
    //     &rpc_client,
    //     &sender_pubkey,
    // );
    ...
    offline.signers = vec![&default_offline_signer];
    offline.command = CliCommand::Transfer {
        amount: SpendAmount::Some(put_to_lamports("0.4")),
        to: recipient_pubkey,
        from: 0,
        sign_only: true,
        dump_transaction_message: false,
    ...
    // rent fee change
    // check_balance!(
    //     put_to_lamports(0.1.to_string()) - 2 * fee_one_sig,
    //     &rpc_client,
    //     &offline_pubkey
    // );
    check_balance!(put_to_lamports("2.9"), &rpc_client, &recipient_pubkey);
}
...
fn test_transfer_multisession_signing() {
    ...
    request_and_confirm_airdrop(
        ...
        put_to_lamports(&43.0.to_string()),
    )
    .unwrap();
    request_and_confirm_airdrop(
        &rpc_client,
        ...
        put_to_lamports(&1.0.to_string()) + 2 * fee,
    )
    .unwrap();
    check_balance!(
        put_to_lamports(&43.0.to_string()),
    ...
    fee_payer_config.command = CliCommand::Transfer {
        amount: SpendAmount::Some(put_to_lamports(&42.0.to_string())),
        to: to_pubkey,
        from: 1,
        sign_only: true,
        dump_transaction_message: false,
    ...
    // rent fee change
    // check_balance!(
    //     put_to_lamports(1.0.to_string()),
    //     &rpc_client,
    //     &offline_from_signer.pubkey(),
    // );
    // check_balance!(
    //     put_to_lamports(1.0.to_string()) + fee,
    //     &rpc_client,
    //     &offline_fee_payer_signer.pubkey(),
    // );
    // check_balance!(put_to_lamports(&42.0.to_string()), &rpc_client, &to_pubkey);
}
...
fn test_transfer_with_seed() {
    ...
    request_and_confirm_airdrop(&rpc_client, &config, &sender_pubkey, put_to_lamports(&1.0.to_string()))
        .unwrap();
    request_and_confirm_airdrop(&rpc_client, &config, &derived_address, put_to_lamports(&5.0.to_string()))
        .unwrap();
    // feature comment
    // rent fee change
    // check_balance!(put_to_lamports(1.0.to_string()), &rpc_client, &sender_pubkey);
    // check_balance!(put_to_lamports(5.0.to_string()), &rpc_client, &derived_address);
    check_balance!(0, &rpc_client, &recipient_pubkey);
    ...
    // Transfer with seed
    config.command = CliCommand::Transfer {
        amount: SpendAmount::Some(put_to_lamports("5.0")),
        to: recipient_pubkey,
        from: 0,
        sign_only: false,
        dump_transaction_message: false,
    ...
    // feature comment
    // rent change
    // check_balance!(put_to_lamports(1.0.to_string()) - fee + 1, &rpc_client, &sender_pubkey);
    // check_balance!(put_to_lamports(5.0.to_string()), &rpc_client, &recipient_pubkey);
