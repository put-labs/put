...
impl AccountsHashes {
    ...
    pub(crate) fn new_rand<R: Rng>(rng: &mut R, pubkey: Option<Pubkey>) -> Self {
        ...
        let hashes = std::iter::repeat_with(|| {
            ...
            let hash = put_sdk::hash::new_rand(rng);
            (slot, hash)
        })
        .take(num_hashes)
        .collect();
...
pub struct LegacyVersion {
    pub from: Pubkey,
    pub wallclock: u64,
    pub version: put_version::LegacyVersion1,
}
...
pub struct Version {
    pub from: Pubkey,
    pub wallclock: u64,
    pub version: put_version::LegacyVersion2,
}
...
impl Version {
    pub fn new(from: Pubkey) -> Self {
        Self {
            from,
            wallclock: timestamp(),
            version: put_version::LegacyVersion2::default(),
        }
    }
    ...
    fn new_rand<R: Rng>(rng: &mut R, pubkey: Option<Pubkey>) -> Self {
        Self {
            from: pubkey.unwrap_or_else(pubkey::new_rand),
            wallclock: new_rand_timestamp(rng),
            version: put_version::LegacyVersion2 {
                major: rng.gen(),
                minor: rng.gen(),
                patch: rng.gen(),
                commit: Some(rng.gen()),
...
mod test {
    use {
        ...
        put_vote_program::{vote_instruction, vote_state},
