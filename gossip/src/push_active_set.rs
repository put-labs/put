use {
    ...
    put_sdk::{native_token::LAMPORTS_PER_PUT, pubkey::Pubkey},
...
fn get_stake_bucket(stake: Option<&u128>) -> usize {
    let stake = stake.copied().unwrap_or_default() / LAMPORTS_PER_PUT;
...
mod tests {
    ...
    fn test_get_stake_bucket() {
        ...
        for (k, bucket) in buckets.into_iter().enumerate() {
            let stake = (k as u128) * LAMPORTS_PER_PUT;
        ...
        for (stake, bucket) in [
            ...
            let stake = stake * LAMPORTS_PER_PUT;
            assert_eq!(get_stake_bucket(Some(&stake)), bucket);
        }
        assert_eq!(
    ...
    fn test_push_active_set() {
        ...
        const MAX_STAKE: u128 = (1 << 20) * LAMPORTS_PER_PUT ;
