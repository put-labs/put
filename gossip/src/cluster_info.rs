...
#[deprecated(
    since = "1.10.6",
    note = "Please use `put_net_utils::{MINIMUM_VALIDATOR_PORT_RANGE_WIDTH, VALIDATOR_PORT_RANGE}` instead"
)]
#[allow(deprecated)]
pub use put_net_utils::{MINIMUM_VALIDATOR_PORT_RANGE_WIDTH, VALIDATOR_PORT_RANGE};
use {
    crate::{
    ...
    put_runtime::{bank_forks::BankForks, vote_parser},
    put_sdk::{
...
const MIN_STAKE_FOR_GOSSIP: u128 = put_sdk::native_token::LAMPORTS_PER_PUT;
...
impl ClusterInfo {
    ...
    pub fn get_node_version(&self, pubkey: &Pubkey) -> Option<put_version::LegacyVersion2> {
        let gossip_crds = self.gossip.crds.read().unwrap();
    ...
    pub fn gossip(
        ...
        let thread_pool = ThreadPoolBuilder::new()
            ...
            .thread_name(|i| format!("putRunGossip{i:02}"))
            .build()
            .unwrap();
        Builder::new()
            .name("putGossip".to_string())
            .spawn(move || {
                let mut last_push = 0;
    ...
    pub(crate) fn start_socket_consume_thread(
        ...
        let thread_pool = ThreadPoolBuilder::new()
            ...
            .thread_name(|i| format!("putGossipCons{i:02}"))
            .build()
            .unwrap();
        ...
        let thread_name = String::from("putGossipConsum");
    ...
    pub(crate) fn listen(
        ...
        let thread_pool = ThreadPoolBuilder::new()
            .num_threads(get_thread_count().min(8))
            .thread_name(|i| format!("putGossipWork{i:02}"))
            .build()
            .unwrap();
        Builder::new()
            .name("putGossipListen".to_string())
            .spawn(move || {
                while !exit.load(Ordering::Relaxed) {
...
mod tests {
    use {
        ...
        regex::Regex,
        put_ledger::shred::Shredder,
        put_net_utils::MINIMUM_VALIDATOR_PORT_RANGE_WIDTH,
        put_sdk::signature::{Keypair, Signer},
        put_vote_program::{vote_instruction, vote_state::Vote},
        std::{
            iter::repeat_with,
    ...
    fn test_gossip_node() {
        ...
        let (node, _, _) = ClusterInfo::spy_node(put_sdk::pubkey::new_rand(), 0);
...
RPC Enabled Nodes: 1"#;
    ...
    fn test_handle_pull() {
        ...
        let entrypoint_pubkey = put_sdk::pubkey::new_rand();
        ...
        let entrypoint_pubkey2 = put_sdk::pubkey::new_rand();
        assert_eq!(
            (1, 0, 0),
    ...
    fn test_cluster_spy_gossip() {
        ...
        let (spy, _, _) = ClusterInfo::spy_node(put_sdk::pubkey::new_rand(), 0);
        let cluster_info = Arc::new({
            let keypair = Arc::new(Keypair::new());
            let node = Node::new_localhost_with_pubkey(&keypair.pubkey());
            ClusterInfo::new(node.info, keypair, SocketAddrSpace::Unspecified)
    ...
    fn insert_info_test() {
        ...
        let d = ContactInfo::new_localhost(&put_sdk::pubkey::new_rand(), timestamp());
    ...
    fn new_vote_transaction<R: Rng>(rng: &mut R, slots: Vec<Slot>) -> Transaction {
        let vote = Vote::new(slots, put_sdk::hash::new_rand(rng));
        let ix = vote_instruction::vote(
            &Pubkey::new_unique(), // vote_pubkey
            &Pubkey::new_unique(), // authorized_voter_pubkey
            vote,
    ...
    fn test_split_gossip_messages() {
        ...
        let splits: Vec<_> =
            ClusterInfo::split_gossip_messages(PUSH_MESSAGE_MAX_PAYLOAD_SIZE, values.clone())
                .collect();
        let self_pubkey = put_sdk::pubkey::new_rand();
    ...
    fn test_pull_from_entrypoint_if_not_present() {
        ...
        let other_node_pubkey = put_sdk::pubkey::new_rand();
    ...
    fn test_process_entrypoint_adopt_shred_version() {
        ...
        let mut gossiped_entrypoint1_info =
            ContactInfo::new_localhost(&put_sdk::pubkey::new_rand(), timestamp());
