...
use {
    ...
    put_bloom::bloom::{AtomicBloom, Bloom},
    put_sdk::{
        hash::{hash, Hash},
...
impl put_sdk::sanitize::Sanitize for CrdsFilter {
    fn sanitize(&self) -> std::result::Result<(), put_sdk::sanitize::SanitizeError> {
    ...
    pub(crate) fn new_pull_request(
        ...
        let (weights, nodes): (Vec<u64>, Vec<ContactInfo>) =
            crds_gossip::dedup_gossip_addresses(nodes, stakes)
                .into_values()
                .map(|(stake, node)| {
                    let stake = stake.min(stake_cap) / LAMPORTS_PER_PUT;
...
pub(crate) mod tests {
    use {
        ...
        put_perf::test_tx::new_test_vote_tx,
        put_sdk::{
    ...
    fn test_hash_as_u64_random() {
        ...
        for _ in 0..100 {
            let hash = put_sdk::hash::new_rand(&mut rng);
    ...
    fn test_crds_filter_set_add() {
        ...
        let hash_values: Vec<_> = repeat_with(|| put_sdk::hash::new_rand(&mut rng))
            .take(1024)
            .collect();
    ...
    fn test_new_pull_request() {
        ...
        let new = ContactInfo::new_localhost(&put_sdk::pubkey::new_rand(), now);
        ping_cache
            .lock()
            .unwrap()
        ...
        let offline = ContactInfo::new_localhost(&put_sdk::pubkey::new_rand(), now);
    ...
    fn test_new_mark_creation_time() {
        ...
        let old = ContactInfo::new_localhost(&put_sdk::pubkey::new_rand(), 0);
        ...
        crds.insert(old.clone(), now, GossipRoute::LocalMessage)
            .unwrap();
        let new = ContactInfo::new_localhost(&put_sdk::pubkey::new_rand(), 0);
    ...
    fn test_process_pull_request_response() {
        ...
        let new = ContactInfo::new_localhost(&put_sdk::pubkey::new_rand(), 1);
        ping_cache.mock_pong(*new.pubkey(), new.gossip().unwrap(), Instant::now());
        let new = CrdsValue::new_unsigned(CrdsData::LegacyContactInfo(new));
        node_crds.insert(new, 0, GossipRoute::LocalMessage).unwrap();

        let mut dest_crds = Crds::default();
        let new_id = put_sdk::pubkey::new_rand();
    ...
    fn test_gossip_purge() {
        ...
        let mut node_crds = Crds::default();
        let entry = CrdsValue::new_unsigned(CrdsData::LegacyContactInfo(
            ContactInfo::new_localhost(&put_sdk::pubkey::new_rand(), 0),
        ));
    ...
    fn test_process_pull_response() {
        ...
        let peer_pubkey = put_sdk::pubkey::new_rand();
