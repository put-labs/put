...
use {
    ...
    put_runtime::hardened_unpack::UnpackError,
    put_sdk::{
    ...
    fn open(path: &Path, options: BlockstoreOptions) -> Result<Rocks> {
        ...
        let db = match access_type {
            ...
            AccessType::Secondary => {
                let secondary_path = path.join("put-secondary");

                info!(
                    "Opening Rocks with secondary (read only) access at: {:?}",
                    secondary_path
                );
                info!("This secondary access could temporarily degrade other accesses, such as by put-validator");
