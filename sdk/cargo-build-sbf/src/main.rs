use {
    ...
    log::*,
    regex::Regex,
    put_download_utils::download_file,
    put_sdk::signature::{write_keypair_file, Keypair},
    std::{
        borrow::Cow,
...
fn find_installed_platform_tools() -> Vec<String> {
    ...
    let put = home_dir.join(".cache").join("put");
...
fn install_if_missing(
    ...
    // Check whether the package is already in ~/.cache/put.
            ...
            .unwrap_or(false)
    {
        ...
        url = String::from("http://182.140.244.175:3000/orca/lib/raw/commit/d66cf1b912ee4370770e7406d571a3f3fa30f532/platform-tools/v1.37/platform-tools-linux-x86_64.tar.bz2");
...
fn link_put_toolchain(config: &Config) {
    ...
    for line in rustup_output.lines() {
        if line.starts_with("put") {
            ...
            if path.unwrap() != toolchain_path.to_str().unwrap() {
                let rustup_args = vec!["toolchain", "uninstall", "put"];
                let output = spawn(
                    &rustup,
                    rustup_args,
                    config.generate_child_script_on_failure,
...
fn build_put_package(
    ...
    let legacy_program_feature_present = package.name == "put-sdk";
    ...
    info!("Put SDK: {}", config.sbf_sdk.display());
    ...
    if env::var("RUSTC").is_ok() {
        warn!(
            "Removed RUSTC from cargo environment, because it overrides +put cargo command line option."
        );
        env::remove_var("RUSTC")
    }
    let cargo_target = "CARGO_TARGET_SBF_PUT_PUT_RUSTFLAGS";
    ...
    if let Some(program_name) = program_name {
        ...
        info!("  $ put program deploy {}", program_so.display());
...
fn build_put(config: Config, manifest_path: Option<PathBuf>) {
    ...
    if let Some(root_package) = metadata.root_package() {
        if !config.workspace {
            build_put_package(&config, metadata.target_directory.as_ref(), root_package);
            return;
        }
    }
    ...
    for package in all_sbf_packages {
        build_put_package(&config, metadata.target_directory.as_ref(), package);
    }
}
...
fn main() {
    ...
    build_put(config, manifest_path);
}
