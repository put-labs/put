...
//!
//! [np]: https://docs.put.com/developing/runtime-facilities/programs#secp256k1-program
//!
...
//! [`ecrecover`] precompile. For that PUT provides the [`secp256k1_recover`]
//! syscall.
//!
//! [secp256k1]: https://en.bitcoin.it/wiki/Secp256k1
//! [`secp256k1_program`]: put_program::secp256k1_program
//! [`secp256k1_recover`]: put_program::secp256k1_recover
...
//!
//! The `put_sdk` crate provides few APIs for building the instructions and
//! transactions necessary for properly using the secp256k1 native program.
//! Many steps must be done manually.
//!
//! The `put_program` crate provides no APIs to assist in interpreting
...
//! and callers should ensure they use the same version of `bincode` as the PUT SDK.
//! This data structure is not provided to PUT programs,
...
//!
//! **The put `secp256k1_recover` function does not prevent signature
//! malleability**. This is in contrast to the Bitcoin secp256k1 library, which
//! does prevent malleability by default. PUT accepts signatures with `S`
...
//! to parse the secp256k1 instruction data from within a PUT program.
//!
//! ```no_run
//! mod secp256k1_defs {
...
//!
//! This example has two components: a PUT program, and an RPC client that
//! sends a transaction to call it. The RPC client will sign a single message,
//! and the PUT program will introspect the secp256k1 instruction to verify
//! that the signer matches a known authorized public key.
//!
//! The PUT program. Note that it uses `libsecp256k1` version 0.7.0 to parse
...
//!
//! ```no_run
//! # use put_sdk::example_mocks::put_rpc_client;
//! use anyhow::Result;
...
//! containing many signatures, and a PUT program that parses them all. This
...
//! transaction. In the _previous_ example, the PUT program asserted that all
//! signatures, messages, and addresses were stored in the instruction at 0. In
//! this next example the PUT program supports signatures, messages, and
...
pub fn new_secp256k1_instruction(
    ...
    Instruction {
        program_id: put_sdk::secp256k1_program::id(),
        accounts: vec![],
...
///
/// `feature_set` is the set of active PUT features. It is used to enable or
