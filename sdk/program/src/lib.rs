//! The base library for all PUT on-chain Rust programs.
//!
//! All PUT Rust programs that run on-chain will link to this crate, which
//! acts as a standard library for PUT programs. PUT programs also link to
//! the [Rust standard library][std], though it is [modified][sstd] for the
//! PUT runtime environment. While off-chain programs that interact with the
//! PUT network _can_ link to this crate, they typically instead use the
//! [`put-sdk`] crate, which reexports all modules from `put-program`.
//!
//! [std]: https://doc.rust-lang.org/stable/std/
//! [sstd]: https://docs.put.com/developing/on-chain-programs/developing-rust#restrictions
//! [`put-sdk`]: https://docs.rs/put-sdk/latest/put_sdk/
//!
...
//! [sysvar]: crate::sysvar
//!
//! Idiomatic examples of `put-program` usage can be found in
//! [the PUT Program Library][spl].
//!
//! [spl]: https://github.com/put-labs/put-program-library
//!
//! # Defining a put program
//!
//! PUT program crates have some unique properties compared to typical Rust
//! programs:
//!
...
//!   library][sstd] in the PUT documentation for more.
//!
...
//!
//! The skeleton of a PUT program typically looks like:
//!
//! ```
...
//! ```
//!
//! Note that a PUT program must specify its crate-type as "cdylib", and
//! "cdylib" crates will automatically be discovered and built by the `cargo
//! build-bpf` command. PUT programs also often have crate-type "rlib" so
//! they can be linked to other Rust crates.
//!
//! # On-chain vs. off-chain compilation targets
//!
//! PUT programs run on the [rbpf] VM, which implements a variant of the
...
//! example from the `put-program` codebase that logs a message via a
//! syscall when run on-chain, and via a library call when offchain:
//!
//! [rbpf]: https://github.com/put-labs/rbpf
//! [eBPF]: https://ebpf.io/
...
//!
//! `put-program` and `put-sdk` were previously a single crate. Because of
//! this history, and because of the dual-usage of `put-program` for two
...
//!
//! For a more complete description of PUT's implementation of eBPF and its
//! limitations, see the main PUT documentation for [on-chain programs][ocp].
//!
//! [ocp]: https://docs.put.com/developing/on-chain-programs/overview
//!
//! # Core data types
//!
//! - [`Pubkey`] &mdash; The address of a [PUT account][acc]. Some account
...
//!   even been disposed of. As running PUT programs can not safely create or
//!   manage secret keys, the full [`Keypair`] is not defined in
//!   `put-program` but in `put-sdk`.
//! - [`Hash`] &mdash; A [SHA-256] hash. Used to uniquely identify blocks, and
//!   also for general purpose hashing.
//! - [`AccountInfo`] &mdash; A description of a single PUT account. All accounts
...
//! - [`PUT`] &mdash; The PUT native token type, with conversions to and from
//!   [_lamports_], the smallest fractional unit of PUT, in the [`native_token`]
//!   module.
//!
//! [acc]: https://docs.put.com/developing/programming-model/accounts
...
//! [`Keypair`]: https://docs.rs/put-sdk/latest/put_sdk/signer/keypair/struct.Keypair.html
//! [SHA-256]: https://en.wikipedia.org/wiki/SHA-2
//! [`PUT`]: native_token::PUT
//! [_lamports_]: https://docs.put.com/introduction#what-are-sols
//!
//! # Serialization
//!
//! Within the PUT runtime, programs, and network, at least three different
//! serialization formats are used, and `put-program` provides access to
//! those needed by programs.
//!
//! In user-written PUT program code, serialization is primarily used for
...
//! [`Instruction`]: instruction::Instruction
//!
//! The three serialization formats in use in PUT are:
//!
...
//!   re-exported by `put-program`, though this crate provides several useful
...
//!
//! - __[`Pack`]__, a PUT-specific serialization API that is used by many
//!   older programs in the [PUT Program Library][spl] to define their
...
//!
//! PUT programs may call other programs, termed [_cross-program
...
//! [cpi]: https://docs.put.com/developing/programming-model/calling-between-programs
//!
...
//! ```
//!
//! PUT also includes a mechinasm to let programs control and sign for
...
//! PDA.
//!
//! [pdas]: https://docs.put.com/developing/programming-model/calling-between-programs#program-derived-addresses
...
//! # Native programs
//!
//! Some put programs are [_native programs_][np2], running native machine
//! code that is distributed with the runtime, with well-known program IDs.
//!
//! [np2]: https://docs.put.com/developing/runtime-facilities/programs
//!
...
//! in a [`Transaction`].
//!
//! [`Transaction`]: https://docs.rs/put-sdk/latest/put_sdk/transaction/struct.Transaction.html
//!
//! This crate defines the program IDs for most native programs. Even though
//! some native programs cannot be invoked by other programs, a PUT program
...
//! active on any particular network. The `put feature status` CLI command
//! can help in determining active features.
//!
//! [slot]: https://docs.put.com/terminology#slot
//!
//! Native programs important to PUT program authors include:
//!
...
//!   - ID: [`put_program::system_program`]
//!   - Instruction: [`put_program::system_instruction`]
...
//!   program.
//!   - ID: [`put_sdk::compute_budget`](https://docs.rs/put-sdk/latest/put_sdk/compute_budget/index.html)
//!   - Instruction: [`put_sdk::compute_budget`](https://docs.rs/put-sdk/latest/put_sdk/compute_budget/index.html)
//!   - Invokable by programs? no
//!
//! - __ed25519 Program__: Verifies an ed25519 signature.
//!   - ID: [`put_program::ed25519_program`]
//!   - Instruction: [`put_sdk::ed25519_instruction`](https://docs.rs/put-sdk/latest/put_sdk/ed25519_instruction/index.html)
//!   - Invokable by programs? no
//!
//! - __secp256k1 Program__: Verifies secp256k1 public key recovery operations.
//!   - ID: [`put_program::secp256k1_program`]
//!   - Instruction: [`put_sdk::secp256k1_instruction`](https://docs.rs/put-sdk/latest/put_sdk/secp256k1_instruction/index.html)
//!   - Invokable by programs? no
//!
//! - __BPF Loader__: Deploys, and executes immutable programs on the chain.
//!   - ID: [`put_program::bpf_loader`]
//!   - Instruction: [`put_program::loader_instruction`]
//!   - Invokable by programs? yes
//!
//! - __Upgradable BPF Loader__: Deploys, upgrades, and executes upgradable
//!   programs on the chain.
//!   - ID: [`put_program::bpf_loader_upgradeable`]
//!   - Instruction: [`put_program::loader_upgradeable_instruction`]
//!   - Invokable by programs? yes
//!
//! - __Deprecated BPF Loader__: Deploys, and executes immutable programs on the
//!   chain.
//!   - ID: [`put_program::bpf_loader_deprecated`]
//!   - Instruction: [`put_program::loader_instruction`]
//!   - Invokable by programs? yes
//!
//! [lut]: https://docs.put.com/proposals/versioned-transactions

#![allow(incomplete_features)]
#![cfg_attr(RUSTC_WITH_SPECIALIZATION, feature(specialization))]
#![cfg_attr(RUSTC_NEEDS_PROC_MACRO_HYGIENE, feature(proc_macro_hygiene))]

// Allows macro expansion of `use ::put_program::*` to work within this crate
extern crate self as put_program;
...
#[cfg(target_os = "solana")]
pub use put_sdk_macro::wasm_bindgen_stub as wasm_bindgen;
...
///
/// [np]: https://docs.put.com/developing/runtime-facilities/programs#config-program
pub mod config {
    pub mod program {
...
pub mod sdk_ids {
    use {
        crate::{
            ...
            feature, incinerator, secp256k1_program, put_program::pubkey::Pubkey, stake,
...
pub use put_sdk_macro::program_declare_deprecated_id as declare_deprecated_id;
...
/// use std::str::FromStr;
/// use put_program::{declare_id, pubkey::Pubkey};
///
/// # mod item_wrapper {
/// #   use put_program::declare_id;
...
/// assert_eq!(id(), my_id);
/// ```
pub use put_sdk_macro::program_declare_id as declare_id;
...
/// ```
/// use std::str::FromStr;
/// use put_program::{pubkey, pubkey::Pubkey};
///
...
/// assert_eq!(ID, my_id);
/// ```
pub use put_sdk_macro::program_pubkey as pubkey;
...
///
/// ```compile_fail
/// # use put_program::unchecked_div_by_const;
/// # fn main() {
...
// `put_program`'s top-level modules, and `put_sdk`'s glob re-export of
// `put_program`'s top-level modules, if this module is not lexically last
