...
/// [`Keypair`]: https://docs.rs/solana-sdk/latest/put_sdk/signer/keypair/struct.Keypair.html
#[wasm_bindgen]
#[repr(transparent)]
#[derive(
    AbiExample,
...
impl Pubkey {
    ...
    ///
    /// [pda]: https://docs.put.com/developing/programming-model/calling-between-programs#program-derived-addresses
    ///
    ...
    /// as a PUT `Pubkey`, except they are ensured to not be on the ed25519
    ...
    ///
    /// As all account addresses accessed by an on-chain PUT program must be
    ...
    /// program, which invokes an on-chain PUT program that uses the address
    ...
    /// instruction as a PUT transaction; and off-chain, as a library, so
    /// that clients can share the instruction data structure, constructors, and
    /// other common code.
    ///
    /// First the on-chain PUT program:
    ///
    /// ```
    ...
    /// ```
    /// # use borsh::{BorshSerialize, BorshDeserialize};
    /// # use put_program::example_mocks::{put_sdk, put_client};
    /// # use put_program::{
    /// #     pubkey::Pubkey,
    ...
    /// # };
    /// # use put_client::rpc_client::RpcClient;
