//! Logging utilities for Rust-based PUT programs.
//!
//! Logging is the main mechanism for getting debugging information out of
//! running PUT programs, and there are several functions available for doing
...
//!
//! - The `put logs` command displays logs for all transactions executed on a
...
//!   active then the `put_client` crate logs at the "debug" level any logs
//!   for transactions that failed during simulation. If using [`env_logger`]
//!   these logs can be activated by setting `RUST_LOG=put_client=debug`.
...
//! [`RpcClient::get_transaction`]: https://docs.rs/put-client/latest/put_client/rpc_client/struct.RpcClient.html#method.get_transaction
//!
...
/// for constrained environments like the PUT VM.
///
/// # Examples
///
/// ```
...
macro_rules! msg {
    ...
    ($($arg:tt)*) => ($crate::log::put_log(&format!($($arg)*)));
}

/// Print a string to the log.
#[inline]
pub fn put_log(message: &str) {
...
#[inline]
pub fn put_log_64(arg1: u64, arg2: u64, arg3: u64, arg4: u64, arg5: u64) {
    #[cfg(target_os = "solana")]
    unsafe {
...
#[inline]
pub fn put_log_128(arg1: u64, arg2: u64, arg3: u64, arg4: u64, arg5: u64) {
    #[cfg(target_arch = "bpf")]
    unsafe {
...
#[allow(dead_code)]
pub fn put_log_slice(slice: &[u8]) {
    for (i, s) in slice.iter().enumerate() {
        put_log_64(0, 0, 0, i as u64, *s as u64);
    }
}
...
#[allow(dead_code)]
pub fn put_log_params(accounts: &[AccountInfo], data: &[u8]) {
    for (i, account) in accounts.iter().enumerate() {
        msg!("AccountInfo");
        put_log_64(0, 0, 0, 0, i as u64);
        msg!("- Is signer");
        put_log_64(0, 0, 0, 0, account.is_signer as u64);
        msg!("- Key");
        account.key.log();
        msg!("- Lamports");
        put_log_128(0, 0, 0,  (account.lamports() >> 64) as u64, (account.lamports() & 0x00000000FFFFFFFF )  as u64);
        msg!("- Account data length");
        put_log_64(0, 0, 0, 0, account.data_len() as u64);
        msg!("- Owner");
        account.owner.log();
    }
