use {
    crate::args::{
        Args, BalancesArgs, Command, DistributeTokensArgs, SenderStakeArgs, PplTokenArgs,
    ...
    put_clap_utils::{
        input_parsers::{pubkey_of_signer, value_of, lamports_of_put},
        input_validators::{is_amount, is_url_or_moniker, is_valid_pubkey, is_valid_signer},
        keypair::{pubkey_from_path, signer_from_path},
    },
    put_cli_config::CONFIG_FILE,
    put_remote_wallet::remote_wallet::maybe_wallet_manager,
    ...
    App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("json_rpc_url")
                ...
                .value_name("URL")
                .help("RPC entrypoint address. i.e. http://rpc.put.com"),
        )
        .subcommand(
            SubCommand::with_name("distribute-tokens")
                .about("Distribute PUT")
                .arg(
                ...
                .arg(
                    Arg::with_name("transfer_amount")
                        ...
                        .value_name("AMOUNT")
                        .validator(is_amount)
                        .help("The amount to send to each recipient, in PUT"),
                )
                .arg(
        ...
        .subcommand(
            SubCommand::with_name("create-stake")
                ...
                .arg(
                    Arg::with_name("unlocked_put")
                        .default_value("1.0")
                        .long("unlocked-put")
                        .takes_value(true)
                        .value_name("PUT_AMOUNT")
                        .help("Amount of PUT to put in system account to pay for fees"),
                )
                .arg(
        ...
        .subcommand(
            SubCommand::with_name("distribute-ppl-tokens")
                .about("Distribute PPL tokens")
                .arg(
                    Arg::with_name("db_path")
                        .long("db-path")
                        .required(true)
                ...
                .arg(
                    Arg::with_name("transfer_amount")
                        ...
                        .value_name("AMOUNT")
                        .validator(is_amount)
                        .help("The amount of PPL tokens to send to each recipient"),
                )
                .arg(
                    Arg::with_name("output_path")
                        .long("output-path")
                ...
                .arg(
                    Arg::with_name("token_account_address")
                        ...
                        .validator(is_valid_pubkey)
                        .help("PPL token account to send from"),
                )
                .arg(
        ...
        .subcommand(
            SubCommand::with_name("ppl-token-balances")
                .about("Balance of PPL token associated accounts")
                .arg(
                    Arg::with_name("input_csv")
                        .long("input-csv")
                        .required(true)
                ...
                .arg(
                    Arg::with_name("mint_address")
                        ...
                        .validator(is_valid_pubkey)
                        .help("PPL token mint of distribution"),
                ),
        )
        .subcommand(
...
fn parse_distribute_tokens_args(
    ...
    Ok(DistributeTokensArgs {
        ...
        stake_args: None,
        ppl_token_args: None,
        transfer_amount: lamports_of_put(matches, "transfer_amount"),
    })
}
...
fn parse_create_stake_args(
    ...
    let stake_args = StakeArgs {
        unlocked_put: lamports_of_put(matches, "unlocked_put").unwrap(),
        lockup_authority,
...
fn parse_distribute_ppl_tokens_args(
    matches: &ArgMatches<'_>,
) -> Result<DistributeTokensArgs, Box<dyn Error>> {
    let mut wallet_manager = maybe_wallet_manager()?;
    let signer_matches = ArgMatches::default(); // No default signer
    ...
    Ok(DistributeTokensArgs {
        ...
        fee_payer,
        stake_args: None,
        ppl_token_args: Some(PplTokenArgs {
            token_account_address,
            ..PplTokenArgs::default()
        }),
...
fn parse_balances_args(matches: &ArgMatches<'_>) -> Result<BalancesArgs, Box<dyn Error>> {
    let mut wallet_manager = maybe_wallet_manager()?;
    let ppl_token_args =
        pubkey_of_signer(matches, "mint_address", &mut wallet_manager)?.map(|mint| PplTokenArgs {
            mint,
            ..PplTokenArgs::default()
        });
    Ok(BalancesArgs {
    ...
    T: Into<OsString> + Clone,
{
    ...
    let command = match matches.subcommand() {
        ...
        ("distribute-ppl-tokens", Some(matches)) => {
            Command::DistributeTokens(parse_distribute_ppl_tokens_args(matches)?)
        }
        ("balances", Some(matches)) => Command::Balances(parse_balances_args(matches)?),
        ("ppl-token-balances", Some(matches)) => Command::Balances(parse_balances_args(matches)?),
