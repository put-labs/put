use {
    ...
    put_account_decoder::parse_token::real_number_string,
    ...
    put_sdk::{
        ...
        message::Message,
        native_token::{lamports_to_put, put_to_lamports},
    ...
    put_transaction_status::TransactionStatus,
    ppl_associated_token_account::get_associated_token_address,
    ppl_token::put_program::program_error::ProgramError,
    std::{
        cmp::{self},
        io,
        sync::{
...
fn distribution_instructions(
    ...
    if args.ppl_token_args.is_some() {
        return build_ppl_token_instructions(allocation, args, do_create_associated_token_account);
    }
    ...
    match &args.stake_args {
        ...
        Some(stake_args) => {
            let unlocked_put = stake_args.unlocked_put;
            ...
            let mut instructions = match &stake_args.sender_stake_args {
                ...
                None => {
                    ...
                    stake_instruction::create_account(
                        ...
                        &authorized,
                        &lockup,
                        allocation.amount - unlocked_put,
                    )
                }
...
fn build_messages(
    ...
    if let Some(spl_token_args) = &args.ppl_token_args {
    ...
    for (i, allocation) in allocations.iter().enumerate() {
        ...
        let do_create_associated_token_account = if let Some(ppl_token_args) = &args.ppl_token_args
        {
            ...
            println!(
                ...
                real_number_string(allocation.amount as u128, ppl_token_args.decimals)
            );
            ...
            println!(
                "{:<44}  {:>24.9}",
                allocation.recipient,
                lamports_to_put(allocation.amount)
            );
            false
        };
...
fn distribute_allocations(
    ...
    if args.ppl_token_args.is_some() {
        check_ppl_token_balances(&messages, allocations, client, args, created_accounts)?;
    } else {
    ...
    let allocations = if let Some(amount) = transfer_amount {
        ...
        recipients
            ...
            .map(|(recipient, amount, lockup_date)| Allocation {
                recipient,
                amount: put_to_lamports(amount.to_string().as_ref()),
                lockup_date,
            })
            .collect()
    } else if raw_amount {
        ...
        recipients
            ...
            .map(|(recipient, amount)| Allocation {
                recipient,
                amount: put_to_lamports(&amount.to_string()),
                lockup_date: "".to_string(),
            })
            .collect()
    };
...
pub fn process_allocations(
    ...
    let mut allocations: Vec<Allocation> = read_allocations(
        ...
        args.ppl_token_args.is_some(),
    )?;

    let starting_total_tokens = allocations.iter().map(|x| x.amount).sum();
    let starting_total_tokens = if let Some(ppl_token_args) = &args.ppl_token_args {
        Token::ppl_token(starting_total_tokens, ppl_token_args.decimals)
    } else {
        Token::put(starting_total_tokens)
    };
    println!(
        "{} {}",
    ...
    let (distributed_tokens, undistributed_tokens) =
        if let Some(ppl_token_args) = &args.ppl_token_args {
            (
                Token::ppl_token(distributed_tokens, ppl_token_args.decimals),
                Token::ppl_token(undistributed_tokens, ppl_token_args.decimals),
            )
        } else {
            (
                Token::put(distributed_tokens),
                Token::put(undistributed_tokens),
            )
        };
...
fn check_payer_balances(
    ...
    let (distribution_source, unlocked_put_source) = if let Some(stake_args) = &args.stake_args {
        let total_unlocked_put = allocations.len() as u128 * stake_args.unlocked_put;
        undistributed_tokens -= total_unlocked_put;
        ...
        (
            from_pubkey,
            Some((args.sender_keypair.pubkey(), total_unlocked_put)),
        )
    } else {
    ...
    if let Some((unlocked_put_source, total_unlocked_put)) = unlocked_put_source {
        ...
        if staker_balance < undistributed_tokens {
            return Err(Error::InsufficientFunds(
                ...
                lamports_to_put(undistributed_tokens).to_string(),
            ));
        }
        if args.fee_payer.pubkey() == unlocked_put_source {
            if fee_payer_balance < fees + total_unlocked_put {
                return Err(Error::InsufficientFunds(
                    vec![FundingSource::SystemAccount, FundingSource::FeePayer].into(),
                    lamports_to_put(fees + total_unlocked_put).to_string(),
                ));
            }
        } else {
            if fee_payer_balance < fees {
                return Err(Error::InsufficientFunds(
                    ...
                    lamports_to_put(fees).to_string(),
                ));
            }
            let unlocked_put_balance = client.get_balance(&unlocked_put_source)?;
            if unlocked_put_balance < total_unlocked_put {
                return Err(Error::InsufficientFunds(
                    vec![FundingSource::SystemAccount].into(),
                    lamports_to_put(total_unlocked_put).to_string(),
                ));
            }
        }
        ...
        if fee_payer_balance < fees + undistributed_tokens {
            return Err(Error::InsufficientFunds(
                vec![FundingSource::SystemAccount, FundingSource::FeePayer].into(),
                lamports_to_put(fees + undistributed_tokens).to_string(),
            ));
        }
    } else {
        if fee_payer_balance < fees {
...
pub fn process_balances(
    ...
    let allocations: Vec<Allocation> =
        read_allocations(&args.input_csv, None, false, args.ppl_token_args.is_some())?;
    let allocations = merge_allocations(&allocations);

    let token = if let Some(ppl_token_args) = &args.ppl_token_args {
        ppl_token_args.mint.to_string()
    } else {
        "◎".to_string()
    };
    ...
    for allocation in &allocations {
        ...
        if let Some(ppl_token_args) = &args.ppl_token_args {
            print_token_balances(client, allocation, ppl_token_args)?;
        } else {
            let address: Pubkey = allocation.recipient.parse().unwrap();
            let expected = lamports_to_put(allocation.amount);
            let actual = lamports_to_put(client.get_balance(&address).unwrap());
            println!(
                "{:<44}  {:>24.9}  {:>24.9}  {:>24.9}",
                allocation.recipient,
                expected,
...
use {
    ...
    put_sdk::{pubkey::Pubkey, signature::Keypair},
...
pub fn test_process_distribute_tokens_with_client(
    ...
    let expected_amount = if let Some(amount) = transfer_amount {
        ...
        put_to_lamports("1000.0")
    };
    let alice_pubkey = put_sdk::pubkey::new_rand();
    ...
    wtr.write_record([
        ...
        lamports_to_put(expected_amount).to_string(),
    ])
    .unwrap();
    wtr.flush().unwrap();
...
pub fn test_process_create_stake_with_client(client: &RpcClient, sender_keypair: Keypair) {
    ...
    let expected_amount = put_to_lamports("1000.0");
    let alice_pubkey = put_sdk::pubkey::new_rand();
    ...
    let stake_args = StakeArgs {
        ...
        unlocked_put: put_to_lamports("1.0"),
    ...
    assert_eq!(
        ...
        expected_amount - put_to_lamports("1.0"),
    );
...
mod tests {
    use {
        ...
        put_transaction_status::TransactionConfirmationStatus,
    };
    ...
    fn test_process_transfer_amount_allocations() {
        ...
        let client = RpcClient::new_with_commitment(url, CommitmentConfig::processed());
        test_process_distribute_tokens_with_client(&client, alice, Some(put_to_lamports("1.5")));
    }
    ...
    fn test_read_allocations() {
        ...
        let allocation_put = Allocation {
            recipient: alice_pubkey.to_string(),
            amount: put_to_lamports("42.0"),
            lockup_date: "".to_string(),
        };

        assert_eq!(
            read_allocations(&input_csv, None, true, true).unwrap(),
            vec![allocation_put.clone()]
        );
        assert_eq!(
    ...
    fn test_read_allocations_transfer_amount() {
        ...
        let amount = put_to_lamports("1.5");
    ...
    fn test_apply_previous_transactions() {
        let alice = put_sdk::pubkey::new_rand();
        let bob = put_sdk::pubkey::new_rand();
        let mut allocations = vec![
            Allocation {
                recipient: alice.to_string(),
                amount: put_to_lamports("1.0"),
                lockup_date: "".to_string(),
            },
            Allocation {
    ...
    fn test_set_split_stake_lockup() {
        ...
        let stake_account_address = put_sdk::pubkey::new_rand();
        let new_stake_account_address = put_sdk::pubkey::new_rand();
    ...
    fn initialize_check_payer_balances_inputs(
        ...
        let recipient = put_sdk::pubkey::new_rand();
        let allocations = vec![Allocation {
    ...
    fn test_check_payer_balances_distribute_tokens_single_payer() {
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            ...
            assert_eq!(amount, (allocation_amount + fees_in_put.parse::<f64>().unwrap()).to_string());
        } else {
        ...
        let transaction = transfer(
            &client,
            put_to_lamports(allocation_amount.to_string().as_ref()),
            &alice,
    ...
    fn test_check_payer_balances_distribute_tokens_separate_payers() {
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            ...
            assert_eq!(amount, fees_in_put.to_string());
        } else {
            panic!("check_payer_balances should have errored");
        }
    }
    ...
    fn test_check_payer_balances_distribute_stakes_single_payer() {
        ...
        let allocation_amount = 1000.0;
        let unlocked_put = 1.0;
        let stake_args = initialize_stake_account(
            put_to_lamports(allocation_amount.to_string().as_ref()),
            put_to_lamports(unlocked_put.to_string().as_ref()),
            &alice,
            &client,
        );
        ...
        let expensive_allocations = vec![Allocation {
            recipient: put_sdk::pubkey::new_rand().to_string(),
            amount: put_to_lamports(expensive_allocation_amount.to_string().as_ref()),
            lockup_date: "".to_string(),
        }];
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            ...
            assert_eq!(
                amount,
                (expensive_allocation_amount - unlocked_put).to_string()
            );
        } else {
            panic!("check_payer_balances should have errored");
        }
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            ...
            assert_eq!(amount, (unlocked_put + fees_in_put.parse::<f64>().unwrap()).to_string());
        } else {
            panic!("check_payer_balances should have errored");
        }
    ...
    fn test_check_payer_balances_distribute_stakes_separate_payers() {
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            assert_eq!(sources, vec![FundingSource::SystemAccount].into());
            assert_eq!(amount, unlocked_put.to_string());
        } else {
            panic!("check_payer_balances should have errored");
        }
    ...
    fn test_build_messages_dump_db() {
        ...
        let amount = put_to_lamports("1.0");
