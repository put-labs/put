...
```bash
put-tokens distribute-tokens --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
put-tokens transaction-log --output-path transactions.csv
```
...
```bash
put-tokens distribute-tokens --dry-run --input-csv <RECIPIENTS_CSV>
```
...
```bash
put-tokens distribute-tokens --transfer-amount 10 --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
put-tokens distribute-stake --stake-account-address <ACCOUNT_ADDRESS> \
    --input-csv <ALLOCATIONS_CSV> \
    --stake-authority <KEYPAIR> --withdraw-authority <KEYPAIR> --fee-payer <KEYPAIR>
```

Currently, this will subtract 1 PUT from each allocation and store it the
recipient address. That PUT can be used to pay transaction fees on staking
...
Distributing SPL Tokens works very similarly to distributing PUT, but requires
...
```bash
put-tokens spl-token-balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
...
```bash
put-tokens distribute-spl-tokens --from <ADDRESS> --owner <KEYPAIR> \
...
As with PUT, you can List the differences between a list of expected
distributions and the record of what transactions have already been sent using
the `--dry-run` parameter, or `put-tokens balances`.
...
```bash
put-tokens balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
