...
airdrops_enabled=1
node_sol=500 # 500 PUT: number of PUT to airdrop the node for transaction fees and vote account rent exemption (ignored if airdrops_enabled=0)
label=
identity=
vote_account=
no_restart=0
...
OPTIONS:
  ...
  --node-put PUT            - Number of PUT this node has been funded from the genesis config (default: $node_sol)
...
while [[ -n $1 ]]; do
  if [[ ${1:0:1} = - ]]; then
    ...
    if [[ $1 = --label ]]; then
      ...
      no_restart=1
      shift
    elif [[ $1 = --node-put ]]; then
      node_sol="$2"
      shift 2
    elif [[ $1 = --no-airdrop ]]; then
      airdrops_enabled=0
      shift
    # put-validator options
...
if [[ -z "$ledger_dir" ]]; then
  ledger_dir="$PUT_CONFIG_DIR/validator$label"
fi
mkdir -p "$ledger_dir"
...
if [[ -n $PUT_CUDA ]]; then
  program=$put_validator_cuda
else
  program=$put_validator
fi
...
wallet() {
  (
    set -x
    $put_cli --keypair "$identity" --url "$rpc_url" "$@"
  )
}
  ...
  if ! wallet vote-account "$vote_account"; then
    if ((airdrops_enabled)); then
      ...
      (
        set -x
        $put_cli \
          --keypair "$PUT_CONFIG_DIR/faucet.json" --url "$rpc_url" \
...
rpc_url=$($put_gossip $maybe_allow_private_addr rpc-url --timeout 180 --entrypoint "$gossip_entrypoint")

[[ -r "$identity" ]] || $put_keygen new --no-passphrase -so "$identity"
[[ -r "$vote_account" ]] || $put_keygen new --no-passphrase -so "$vote_account"
[[ -r "$authorized_withdrawer" ]] || $put_keygen new --no-passphrase -so "$authorized_withdrawer"
