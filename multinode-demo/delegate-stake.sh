...
stake_sol=1   # default number of PUT to assign as stake (1 PUT)
...
usage: $0 [OPTIONS] <PUT to stake ($stake_sol)>
...
VALIDATOR_KEYS_DIR=$PUT_CONFIG_DIR/validator$label
...
if ((airdrops_enabled)); then
  ...
  $put_cli \
    "${common_args[@]}" --keypair "$PUT_CONFIG_DIR/faucet.json" \
...
if ! [[ -f "$stake_account" ]]; then
  $put_keygen new --no-passphrase -so "$stake_account"
else
...
$put_cli "${common_args[@]}" stakes "$stake_account"
