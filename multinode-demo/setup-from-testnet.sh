...
rm -rf "$PUT_CONFIG_DIR"/latest-testnet-snapshot
mkdir -p "$PUT_CONFIG_DIR"/latest-testnet-snapshot
(
  cd "$PUT_CONFIG_DIR"/latest-testnet-snapshot || exit 1
  set -x
  wget http://test_rpc.put.com/genesis.tar.bz2
  wget --trust-server-names http://testnet.put.com/snapshot.tar.bz2
)

snapshot=$(ls "$PUT_CONFIG_DIR"/latest-testnet-snapshot/snapshot-[0-9]*-*.tar.zst)
if [[ -z $snapshot ]]; then
...
$put_ledger_tool create-snapshot \
  --ledger "$PUT_CONFIG_DIR"/latest-testnet-snapshot \
