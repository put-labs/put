...
if [[ -f $BOOTSTRAP_VALIDATOR_STAKE_KEYPAIR ]]; then
  cp -f "$BOOTSTRAP_VALIDATOR_STAKE_KEYPAIR" "$PUT_CONFIG_DIR"/bootstrap-validator/stake-account.json
else
...
if [[ -f $BOOTSTRAP_VALIDATOR_VOTE_KEYPAIR ]]; then
  cp -f "$BOOTSTRAP_VALIDATOR_VOTE_KEYPAIR" "$PUT_CONFIG_DIR"/bootstrap-validator/vote-account.json
else
...
"$PUT_ROOT"/fetch-spl.sh
if [[ -r spl-genesis-args.sh ]]; then
  SPL_GENESIS_ARGS=$(cat "$PUT_ROOT"/spl-genesis-args.sh)
  #shellcheck disable=SC2207
  #shellcheck disable=SC2206
  args+=($SPL_GENESIS_ARGS)
fi

default_arg --ledger "$PUT_CONFIG_DIR"/bootstrap-validator
default_arg --faucet-pubkey "$PUT_CONFIG_DIR"/faucet.json
