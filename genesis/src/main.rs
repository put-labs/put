...
use {
    ...
    put_entry::poh::compute_hashes_per_tick,
    put_genesis::{Base64Account},
    put_ledger::{blockstore::create_new_ledger, blockstore_options::LedgerColumnOptions},
    ...
    put_vote_program::vote_state::{self, VoteState},
    std::{
        collections::HashMap,
        error,
        fs::File,
...
fn main() -> Result<(), Box<dyn error::Error>> {
    // let default_faucet_pubkey = put_cli_config::Config::default().keypair_path;
    ...
    put_stake_program::add_genesis_accounts(&mut genesis_config);
    // // oa comment activate_all_features
    // oa 1.1.0 comment activate_all_features
    // if genesis_config.cluster_type == ClusterType::Development {
    //     put_runtime::genesis_utils::activate_all_features(&mut genesis_config);
    // }else{
    //     put_runtime::genesis_utils::activate_init_features(&mut genesis_config);
    // }
...
mod tests {
    ...
    fn test_append_primordial_accounts_to_genesis() {
        ...
        genesis_accounts.insert(
            put_sdk::pubkey::new_rand().to_string(),
            Base64Account {
                owner: put_sdk::pubkey::new_rand().to_string(),
                balance: 2,
                executable: false,
