...
impl TryFrom<AccountSharedData> for VoteAccountInner {
    type Error = Error;
    fn try_from(account: AccountSharedData) -> Result<Self, Self::Error> {
        if !put_vote_program::check_id(account.owner()) {
...
mod tests {
    use {
        ...
        bincode::Options,
        rand::Rng,
        put_sdk::{pubkey::Pubkey, sysvar::clock::Clock},
        put_vote_program::vote_state::{VoteInit, VoteStateVersions},
