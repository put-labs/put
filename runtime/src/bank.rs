...
use ethnum::U256;
#[allow(deprecated)]
use put_sdk::recent_blockhashes_account;
pub use put_sdk::reward_type::RewardType;
use {
    crate::{
        ...
        // inline_ppl_associated_token_account, 
        inline_ppl_token,
    ...
    put_bpf_loader_program::syscalls::create_program_runtime_environment,
    put_measure::{measure, measure::Measure, measure_us},
    put_perf::perf_libs,
    put_program_runtime::{
    ...
    put_system_program::{get_system_account_kind, SystemAccountKind},
    put_vote_program::vote_state::{VoteState, VoteStateVersions},
    std::{
        borrow::Cow,
        cell::RefCell,
...
use put_frozen_abi::abi_example::AbiExample;
...
impl Bank {
    ...
    fn filter_stake_delegations<'a>(
            ...
            .is_active(&feature_set::stake_minimum_delegation_for_rewards::id())
        {
            ...
            let min_stake_delegation =
                put_stake_program::get_minimum_delegation(&self.feature_set)
                    .max(LAMPORTS_PER_PUT);
    ...
    fn _load_vote_and_stake_accounts(
        ...
        let put_vote_program: Pubkey = put_vote_program::id();
        ...
        let get_vote_account = |vote_pubkey: &Pubkey| -> Option<VoteAccount> {
            ...
            if account.owner() == &put_vote_program
        ...
        let make_vote_delegations_entry = |vote_pubkey| {
            ...
            if vote_account.owner() != &put_vote_program {
        ...
        let push_stake_delegation = |(stake_pubkey, stake_account): (&Pubkey, &StakeAccount<_>)| {
            ...
            if let Some(reward_calc_tracer) = reward_calc_tracer.as_ref() {
                let delegation =
                    InflationPointCalculationEvent::Delegation(delegation, put_vote_program);
    ...
    fn program_modification_slot(&self, pubkey: &Pubkey) -> Result<Slot> {
        ...
        if bpf_loader_upgradeable::check_id(program.owner()) {
            ...
            let state = put_loader_v4_program::get_state(program.data())
    ...
    pub fn load_program(&self, pubkey: &Pubkey) -> Arc<LoadedProgram> {
        ...
        let program_runtime_environment_v1 = self
            ...
            .clone();
        put_bpf_loader_program::load_program_from_account(
            &self.feature_set,
            None, // log_collector
            &program,
    ...
    pub fn calculate_fee(
        ...
        _use_default_units_per_instruction: bool,
        _support_request_units_deprecated: bool,
        _remove_congestion_multiplier: bool,
        _enable_request_heap_frame_ix: bool,
        _support_set_accounts_data_size_limit_ix: bool,
    ...
    fn collect_rent_from_accounts(
        ...
        let set_exempt_rent_epoch_max: bool = self
            .feature_set
            .is_active(&put_sdk::feature_set::set_exempt_rent_epoch_max::id());
    ...
    fn use_multi_epoch_collection_cycle(&self, epoch: Epoch) -> bool {
        ...
        #[cfg(not(test))]
        if self.slot_count_per_normal_epoch() == put_sdk::epoch_schedule::MINIMUM_SLOTS_PER_EPOCH {
            return false;
        }
    ...
    fn reconfigure_token2_native_mint(&mut self) {
        ...
        if reconfigure_token2_native_mint {
            let mut native_mint_account = put_sdk::account::AccountSharedData::from(Account {
                owner: inline_ppl_token::id(),
                data: inline_ppl_token::native_mint::ACCOUNT_DATA.to_vec(),
                lamports: put_to_lamports("1"),
                executable: false,
                rent_epoch: self.epoch() + 1,
            });

            let mut native_mint_info_account = put_sdk::account::AccountSharedData::from(Account {
                owner: inline_ppl_token::id(),
                data: inline_ppl_token::native_mint_info::ACCOUNT_DATA.to_vec(),
                lamports: put_to_lamports("1"),
                executable: false,
                rent_epoch: self.epoch() + 1,
            });
            ...
            // As a workaround for
            // https://github.com/put-labs/put-program-library/issues/374, ensure that the
            // ppl-token 2 native mint account is owned by the ppl-token 2 program.
            let store = if let Some(existing_native_mint_account) =
                self.get_account_with_fixed_root(&inline_ppl_token::native_mint::id())
            {
                if existing_native_mint_account.owner() == &put_sdk::system_program::id() {
            ...
            if store {
                self.store_account(&inline_ppl_token::native_mint::id(), &native_mint_account);
            }

            // as mint account data
            let _info_store = if let Some(existing_native_mint_info_account) =
                self.get_account_with_fixed_root(&inline_ppl_token::native_mint_info::id())
            {
                if existing_native_mint_info_account.owner() == &put_sdk::system_program::id() {
            ...
            if _info_store {
                self.store_account(
                    &inline_ppl_token::native_mint_info::id(),
                    &native_mint_info_account,
                );
            }
        }
...
pub mod test_utils {
    use {
        super::Bank,
        put_sdk::{hash::hashv, pubkey::Pubkey},
        put_vote_program::vote_state::{self, BlockTimestamp, VoteStateVersions},
    };
