use {
    crate::{
        ...
        inline_ppl_token::{self, GenericTokenAccount},
        inline_ppl_token_2022,
...
pub struct AccountsIndex<T: IndexValue, U: DiskIndexValue + From<T> + Into<T>> {
    ...
    ppl_token_mint_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
    ppl_token_owner_index: SecondaryIndex<RwLockSecondaryIndexEntry>,
    ppl_nft_owner_index: SecondaryIndex<RwLockSecondaryIndexEntry>,
    ppl_name_owner_index: SecondaryIndex<RwLockSecondaryIndexEntry>,
...
impl<T: IndexValue, U: DiskIndexValue + From<T> + Into<T>> AccountsIndex<T, U> {
    ...
    pub fn new(config: Option<AccountsIndexConfig>, exit: &Arc<AtomicBool>) -> Self {
        ...
        Self {
            ...
            ppl_token_mint_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new(
                "ppl_token_mint_index_stats",
            ),
            ppl_token_owner_index: SecondaryIndex::<RwLockSecondaryIndexEntry>::new(
                "ppl_token_owner_index_stats",
            ),
            ppl_nft_owner_index: SecondaryIndex::<RwLockSecondaryIndexEntry>::new(
                "ppl_nft_owner_index_stats",
            ),
            ppl_name_owner_index: SecondaryIndex::<RwLockSecondaryIndexEntry>::new(
                "ppl_name_owner_index_stats",
            ),
        ...
        R: RangeBounds<Pubkey> + std::fmt::Debug,
    {
        ...
        */
        match scan_type {
            ...
            ScanTypes::Indexed(IndexKey::PplTokenMint(mint_key)) => {
                self.do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.ppl_token_mint_index,
                    &mint_key,
                    Some(max_root),
                    config,
                );
            }
            ScanTypes::Indexed(IndexKey::PplTokenOwner(owner_key)) => {
                self.do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.ppl_token_owner_index,
                    &owner_key,
                    Some(max_root),
                    config,
                );
            }
            ScanTypes::Indexed(IndexKey::PplNftOwner(owner_key)) => {
                self.do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.ppl_nft_owner_index,
                    &owner_key,
                    Some(max_root),
                    config,
                );
            }
            ScanTypes::Indexed(IndexKey::PplNameOwner(owner_key)) => {
                self.do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.ppl_name_owner_index,
                    &owner_key,
                    Some(max_root),
                    config,
                );
    ...
    fn update_ppl_token_secondary_indexes<G: GenericTokenAccount>(
        &self,
        token_id: &Pubkey,
        pubkey: &Pubkey,
        ...
        if *account_owner == *token_id {
            if account_indexes.contains(&AccountIndex::PplTokenOwner) {
                if let Some(owner_key) = G::unpack_account_owner(account_data) {
                    if account_indexes.include_key(owner_key) {
                        self.ppl_token_owner_index.insert(owner_key, pubkey);
                    }
                }
            }

            if account_indexes.contains(&AccountIndex::PplTokenMint) {
                if let Some(mint_key) = G::unpack_account_mint(account_data) {
                    if account_indexes.include_key(mint_key) {
                        self.ppl_token_mint_index.insert(mint_key, pubkey);
                    }
                }
            }
        }
    ...
    fn update_nft_secondary_indexes<G: GenericNFTAccount>(
        ...
        if *account_owner == *token_id {
            if account_indexes.contains(&AccountIndex::PplNftOwner) {
                if let Some(owner_key) = G::unpack_account_owner(account_data) {
                    if account_indexes.include_key(owner_key) {
                        self.ppl_nft_owner_index.insert(owner_key, pubkey);
                    }
                }
            }
        }
    ...
    pub fn get_index_key_size(&self, index: &AccountIndex, index_key: &Pubkey) -> Option<usize> {
        match index {
            ...
            AccountIndex::PplTokenOwner => self
                .ppl_token_owner_index
                .index
                .get(index_key)
                .map(|x| x.len()),
            AccountIndex::PplTokenMint => self
                .ppl_token_mint_index
                .index
                .get(index_key)
                .map(|x| x.len()),
            AccountIndex::PplNftOwner => todo!(),
            AccountIndex::PplNameOwner => todo!(),
        }
    }
    ...
    pub(crate) fn log_secondary_indexes(&self) {
        ...
        if !self.ppl_token_mint_index.index.is_empty() {
            info!("secondary index: {:?}", AccountIndex::PplTokenMint);
            self.ppl_token_mint_index.log_contents();
        }
        if !self.ppl_token_owner_index.index.is_empty() {
            info!("secondary index: {:?}", AccountIndex::PplTokenOwner);
            self.ppl_token_owner_index.log_contents();
        }
    }
    ...
    pub(crate) fn update_secondary_indexes(
        ...
        // filtered out by post-scan filters, like in `get_filtered_ppl_token_accounts_by_owner()`.

        self.update_ppl_token_secondary_indexes::<inline_ppl_token::Account>(
            &inline_ppl_token::id(),
            pubkey,
            account_owner,
            account_data,
        ...
        self.update_ppl_token_secondary_indexes::<inline_ppl_token_2022::Account>(
    ...
    fn purge_secondary_indexes_by_inner_key(
        ...
        if account_indexes.contains(&AccountIndex::PplTokenOwner) {
            self.ppl_token_owner_index.remove_by_inner_key(inner_key);
        }

        if account_indexes.contains(&AccountIndex::PplTokenMint) {
            self.ppl_token_mint_index.remove_by_inner_key(inner_key);
        }
    }
...
pub mod tests {
    ...
    pub fn ppl_token_mint_index_enabled() -> AccountSecondaryIndexes {
        let mut account_indexes = HashSet::new();
        account_indexes.insert(AccountIndex::PplTokenMint);
    ...
    pub fn ppl_token_owner_index_enabled() -> AccountSecondaryIndexes {
        let mut account_indexes = HashSet::new();
        account_indexes.insert(AccountIndex::PplTokenOwner);
        AccountSecondaryIndexes {
            indexes: account_indexes,
            keys: None,
        }
    ...
    fn create_dashmap_secondary_index_state() -> (usize, usize, AccountSecondaryIndexes) {
        {
            ...
            let _type_check = SecondaryIndexTypes::DashMap(&index.ppl_token_mint_index);
        }

        (0, PUBKEY_BYTES, ppl_token_mint_index_enabled())
    }
    ...
    fn create_rwlock_secondary_index_state() -> (usize, usize, AccountSecondaryIndexes) {
        {
            // Check that we're actually testing the correct variant
            let index = AccountsIndex::<bool, bool>::default_for_tests();
            let _type_check = SecondaryIndexTypes::RwLock(&index.ppl_token_owner_index);
        }

        (
            PPL_TOKEN_ACCOUNT_OWNER_OFFSET,
            PPL_TOKEN_ACCOUNT_OWNER_OFFSET + PUBKEY_BYTES,
            ppl_token_owner_index_enabled(),
        )
    }
    ...
    fn setup_accounts_index_keys(num_pubkeys: usize) -> (AccountsIndex<bool, bool>, Vec<Pubkey>) {
        ...
        let mut pubkeys: Vec<Pubkey> = std::iter::repeat_with(|| {
            let new_pubkey = put_sdk::pubkey::new_rand();
            index.upsert(
                root_slot,
                root_slot,
                &new_pubkey,
    ...
    >(
        ...
        let mut account_data = vec![0; inline_ppl_token::Account::get_packed_len()];
    ...
    fn run_test_ppl_token_secondary_indexes<
        SecondaryIndexEntryType: SecondaryIndexEntry + Default + Sync + Send,
    >(
        token_id: &Pubkey,
        index: &AccountsIndex<bool, bool>,
    ...
    fn test_dashmap_secondary_index() {
        let (key_start, key_end, secondary_indexes) = create_dashmap_secondary_index_state();
        let index = AccountsIndex::<bool, bool>::default_for_tests();
        for token_id in [inline_ppl_token::id(), inline_ppl_token_2022::id()] {
            run_test_ppl_token_secondary_indexes(
                &token_id,
                &index,
                &index.ppl_token_mint_index,
                key_start,
    ...
    >(
        ...
        let slot = 1;
        let mut account_data1 = vec![0; inline_ppl_token::Account::get_packed_len()];
        account_data1[index_key_start..index_key_end]
            .clone_from_slice(&(secondary_key1.to_bytes()));
        let mut account_data2 = vec![0; inline_ppl_token::Account::get_packed_len()];
    ...
    fn test_clean_rooted_entries_return() {
        ...
        let value = true;
        let key = put_sdk::pubkey::new_rand();
        let key_unknown = put_sdk::pubkey::new_rand();
        let index = AccountsIndex::<bool, bool>::default_for_tests();
        let slot1 = 1;
