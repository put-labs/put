use {
    ...
    log::*,
    put_clap_utils::input_parsers::{lamports_of_put, value_of},
    put_faucet::{
        faucet::{run_faucet, Faucet, FAUCET_PORT},
        socketaddr,
    },
    put_sdk::signature::read_keypair_file,
    std::{
        collections::HashSet,
...
#[tokio::main]
async fn main() {
    let default_keypair = put_cli_config::Config::default().keypair_path;

    put_logger::setup_with_default("put=info");
    put_metrics::set_panic_hook("faucet", /*version:*/ None);
    let matches = App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("per_time_cap")
                ...
                .value_name("NUM")
                .takes_value(true)
                .help("Request limit for time slice, in PUT"),
        )
        .arg(
            Arg::with_name("per_request_cap")
                ...
                .value_name("NUM")
                .takes_value(true)
                .help("Request limit for a single request, in PUT"),
        )
        .arg(
    ...
    let per_time_cap = lamports_of_put(&matches, "per_time_cap");
    let per_request_cap = lamports_of_put(&matches, "per_request_cap");
