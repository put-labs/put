//! The `faucet` module provides an object for launching a PUT Faucet,
//! which is the custodian of any remaining lamports in a mint.
//! The PUT Faucet builds and sends airdrop transactions,
...
impl Faucet {
    ...
    pub fn new_with_allowed_ips(
        ...
        if let Some((per_request_cap, per_time_cap)) = per_request_cap.zip(per_time_cap) {
            if per_time_cap < per_request_cap {
                warn!(
                    "per_time_cap {} PUT < per_request_cap {} PUT; \
                    maximum single requests will fail",
                    lamports_to_put(per_time_cap),
                    lamports_to_put(per_request_cap),
                );
            }
        }
        Self {
        ...
        if let Some(cap) = self.per_time_cap {
            if new_total > cap {
                return Err(FaucetError::PerTimeCapExceeded(
                    lamports_to_put(request_amount),
                    to.to_string(),
                    lamports_to_put(new_total),
                    lamports_to_put(cap),
                ));
            }
        }
    ...
    /// the request exceeds this per-request limit, this method returns a signed PPL Memo
    /// transaction with the memo: "request too large; req: <REQUEST> PUT cap: <CAP> PUT"
    pub fn build_airdrop_transaction(
        ...
        match req {
            FaucetRequest::GetAirdrop {
                ...
                info!(
                    "Requesting airdrop of {} PUT to {:?}",
                    lamports_to_put(lamports),
                    to
                );
                ...
                if let Some(cap) = self.per_request_cap {
                    if lamports > cap {
                        ...
                        let memo_instruction = Instruction {
                            program_id: Pubkey::from(ppl_memo::id().to_bytes()),
                            accounts: vec![],
...
#[cfg(test)]
mod tests {
    use {super::*, put_sdk::system_instruction::SystemInstruction, std::time::Duration};
    ...
    fn test_faucet_build_airdrop_transaction() {
            ...
            faucet.build_airdrop_transaction(request, ip).unwrap()
        {
            ...
            assert_eq!(
                message.account_keys,
                vec![mint_pubkey, Pubkey::from(ppl_memo::id().to_bytes())]
            );
            assert_eq!(message.recent_blockhash, blockhash);
