...
fn is_memo_program(k: &Pubkey) -> bool {
    ...
    // (k_str == ppl_memo_v1_id().to_string()) || (k_str == ppl_memo_id().to_string())
    k_str == ppl_memo_id().to_string()
}
...
fn write_instruction<'a, W: io::Write>(
    ...
    if let AccountKeyType::Known(program_pubkey) = program_pubkey {
        if program_pubkey == &put_vote_program::id() {
            if let Ok(vote_instruction) = limited_deserialize::<
                put_vote_program::vote_instruction::VoteInstruction,
            >(&instruction.data)
            {
        ...
        } else if program_pubkey == &put_sdk::system_program::id() {
            if let Ok(system_instruction) = limited_deserialize::<
                put_sdk::system_instruction::SystemInstruction,
            >(&instruction.data)
            {
        ...
        if !rewards.is_empty() {
            ...
            for reward in rewards {
                ...
                writeln!(
                    ...
                    sign,
                    lamports_to_put(reward.lamports.abs() as u128),
...
fn write_fees<W: io::Write>(w: &mut W, transaction_fee: u128, prefix: &str) -> io::Result<()> {
    writeln!(w, "{}  Fee: ◎{}", prefix, lamports_to_put(transaction_fee))
}
...
mod test {
    use {
        ...
        put_transaction_status::{Reward, RewardType, TransactionStatusMeta},
        std::io::BufWriter,
    };
