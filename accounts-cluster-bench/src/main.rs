...
use {
    ...
    rand::{thread_rng, Rng},
    rayon::prelude::*,
    put_account_decoder::parse_token::{ppl_token_pubkey},
    put_clap_utils::{
        hidden_unless_forced, input_parsers::pubkey_of, input_validators::is_url_or_moniker,
    },
    put_cli_config::{ConfigInput, CONFIG_FILE},
    put_client::transaction_executor::TransactionExecutor,
    put_gossip::gossip_service::discover,
    put_rpc_client::rpc_client::RpcClient,
    put_runtime::inline_ppl_token,
    put_sdk::{
            ...
            if let Some(mint_address) = mint {
                instructions.push(ppl_token_instruction(
                    ppl_token::instruction::initialize_account(
                        &ppl_token::id(),
                        &ppl_token_pubkey(&to_pubkey),
                        &ppl_token_pubkey(&mint_address),
                        &ppl_token_pubkey(&base_keypair.pubkey()),
                    )
                    .unwrap(),
                ));
            }
    ...
    let instructions: Vec<_> = (0..num_instructions)
        .filter_map(|_| {
            ...
            let address =
                Pubkey::create_with_seed(&base_keypair.pubkey(), &seed, &program_id).unwrap();
            if ppl_token {
                Some(ppl_token_instruction(
                    ppl_token::instruction::close_account(
                        &ppl_token::id(),
                        &ppl_token_pubkey(&address),
                        &ppl_token_pubkey(&keypair.pubkey()),
                        &ppl_token_pubkey(&base_keypair.pubkey()),
                        &[],
                    )
                    .unwrap(),
...
fn main() {
    ...
    let client = if let Some(addr) = matches.value_of("entrypoint") {
        let entrypoint_addr = put_net_utils::parse_host_port(addr).unwrap_or_else(|e| {
...
pub mod test {
    use {
        super::*,
        put_core::validator::ValidatorConfig,
        ...
        put_sdk::{native_token::put_to_lamports, poh_config::PohConfig},
    ...
    #[test] // todo oa
    fn test_create_then_reclaim_ppl_token_accounts() {
        put_logger::setup();
        ...
        // Create Mint
        let ppl_mint_keypair = Keypair::new();
        let (mint_meta ,_) = Pubkey::find_program_address(&[b"MintMeta",&ppl_mint_keypair.pubkey().to_bytes()], &ppl_token::id());
        let ppl_mint_len = Mint::get_packed_len();
        let ppl_mint_rent = rpc_client
            .get_minimum_balance_for_rent_exemption(ppl_mint_len)
            .unwrap();
        let transaction = Transaction::new_signed_with_payer(
            &[
                system_instruction::create_account(
                    &funder.pubkey(),
                    &ppl_mint_keypair.pubkey(),
                    ppl_mint_rent,
                    ppl_mint_len as u64,
                    &inline_ppl_token::id(),
                ),
                ppl_token_instruction(
                ...
                ppl_token_instruction(
                    ppl_token::instruction::initialize_mint(
                        &ppl_token::id(),
                        &ppl_token_pubkey(&ppl_mint_keypair.pubkey()),
                        &ppl_token_pubkey(&ppl_mint_keypair.pubkey()),
                        None,
                        2,
                    )
