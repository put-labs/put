//! The put-program-test provides a BanksClient-based test framework BPF programs
...
use {
    ...
    log::*,
    put_banks_client::start_client,
    put_banks_server::banks_server::start_local_server,
    put_bpf_loader_program::serialization::serialize_parameters,
    put_program_runtime::{
    ...
    put_vote_program::vote_state::{self, VoteState, VoteStateVersions},
    std::{
        cell::RefCell,
...
// Export types so test clients can limit their put crate dependencies
pub use {
    put_banks_client::{BanksClient, BanksClientError},
    put_banks_interface::BanksTransactionResultWithMetadata,
    put_program_runtime::invoke_context::InvokeContext,
    put_sdk::transaction_context::IndexOfAccount,
};
...
pub fn builtin_process_instruction(
    process_instruction: put_sdk::entrypoint::ProcessInstruction,
...
/// Converts a `put-program`-style entrypoint into the runtime's entrypoint style, for
...
struct SyscallStubs {}
impl put_sdk::program_stubs::SyscallStubs for SyscallStubs {
...
impl Default for ProgramTest {
    ...
    ///
    fn default() -> Self {
        put_logger::setup_with_default(
            "put_rbpf::vm=debug,\
             put_runtime::message_processor=debug,\
             put_runtime::system_instruction_processor=trace,\
             put_program_test=info",
        );
        let prefer_bpf =
        ...
        let deactivate_feature_set =
            HashSet::from([put_sdk::feature_set::native_programs_consume_cu::id()]);
...
impl ProgramTest {
    ...
    fn setup_bank(
    ...
    ) {
        {
            ...
            ONCE.call_once(|| {
                put_sdk::program_stubs::set_syscall_stubs(Box::new(SyscallStubs {}));
            });
        }
        ...
        let bootstrap_validator_stake_lamports =
            rent.minimum_balance(VoteState::size_of()) + put_to_lamports("1000000.0");

        let mint_keypair = Keypair::new();
        let voting_keypair = Keypair::new();

        let mut genesis_config = create_genesis_config_with_leader_ex(
            put_to_lamports("1000000.0"),
            &mint_keypair.pubkey(),
        ...
        // Add commonly-used PPL programs as a convenience to the user
        for (program_id, account) in programs::ppl_programs(&Rent::default()).iter() {
    ...
    pub fn warp_to_slot(&mut self, warp_slot: Slot) -> Result<(), ProgramTestError> {
        ...
        let warp_bank = if pre_warp_slot == working_slot {
            ...
            bank_forks.insert(Bank::warp_from_parent(
                ...
                put_runtime::accounts_db::CalcAccountsHashDataSource::IndexForTests,
            ))
        };
