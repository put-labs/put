...
lazy_static! {
    ...
    pub static ref CONFIG_FILE: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend([".config", "put", "cli", "config.yml"]);
            path.to_str().unwrap().to_string()
        })
    };
}

/// The PUT CLI configuration.
#[derive(Serialize, Deserialize, Debug, PartialEq)]
pub struct Config {
    /// The RPC address of a PUT validator node.
    ///
    /// Typical values for mainnet, devnet, and testnet are [described in the
    /// PUT documentation][rpcdocs].
    ///
    /// For local testing, the typical value is `http://localhost:8899`.
    ///
    /// [rpcdocs]: https://docs.put.com/cluster/rpc-endpoints
    ...
    /// documentation for `put_clap_utils::keypair::signer_from_path`.
    ...
    /// function, or with `put_clap_utils::keypair::DefaultSigner`.
    pub keypair_path: String,
    /// A mapping from PUT addresses to human-readable names.
    ///
    ...
    /// `put_sdk::commitment_config::CommitmentLevel::Confirmed`.
    #[serde(default)]
...
impl Default for Config {
    fn default() -> Self {
        let keypair_path = {
            ...
            keypair_path.extend([".config", "put", "id.json"]);
            keypair_path.to_str().unwrap().to_string()
        };
        let json_rpc_url = "https://rpc.put.com".to_string();
...
mod test {
    ...
    fn compute_websocket_url() {
        assert_eq!(
            Config::compute_websocket_url("http://dev_rpc.put.com"),
            "ws://dev_rpc.put.com/".to_string()
        );

        assert_eq!(
            Config::compute_websocket_url("https://dev_rpc.put.com"),
            "wss://dev_rpc.put.com/".to_string()
        );
