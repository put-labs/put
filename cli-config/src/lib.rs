//! Loading and saving the PUT CLI configuration file.
//!
//! The configuration file used by the PUT CLI includes information about the
//! RPC node to connect to, the path to the user's signing source, and more.
//! Other software than the PUT CLI may wish to access the same configuration
//! and signer.
//!
...
//!   the documentation for `put_clap_utils::keypair::signer_from_path`.
//!
...
//! ```no_run
//! use anyhow::anyhow;
//! use put_cli_config::{CONFIG_FILE, Config};
//!
//! let config_file = put_cli_config::CONFIG_FILE.as_ref()
//!     .ok_or_else(|| anyhow!("unable to get config file path"))?;
//! let mut cli_config = Config::load(&config_file)?;
//! // Set the RPC URL to devnet
//! cli_config.json_rpc_url = "https://dev_rpc.put.com".to_string();
