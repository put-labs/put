...
pub use put_perf::report_target_features;
use {
    crate::{
    ...
    put_entry::poh::compute_hash_time_ns,
    put_geyser_plugin_manager::{
    ...
    put_metrics::{datapoint_info, poh_timing_point::PohTimingSender},
    put_poh::{
    ...
    put_streamer::{socket::SocketAddrSpace, streamer::StakedNodes},
    put_vote_program::vote_state,
    std::{
...
impl BlockstoreRootScan {
    fn new(config: &ValidatorConfig, blockstore: &Arc<Blockstore>, exit: &Arc<AtomicBool>) -> Self {
            ...
            && config.rpc_config.rpc_scan_and_fix_roots
        {
            ...
            Some(
                Builder::new()
                    .name("putBStoreRtScan".to_string())
...
pub struct Validator {
    ...
    tpu: Tpu,
    tvu: Tvu,
    ip_echo_server: Option<put_net_utils::IpEchoServer>,
...
impl Validator {
    ...
    pub fn new(
        ...
        if rayon::ThreadPoolBuilder::new()
            .thread_name(|i| format!("putRayonGlob{i:02}"))
            .build_global()
            .is_err()
        {
        ...
        if put_perf::perf_libs::api().is_some() {
        ...
        *start_progress.write().unwrap() = ValidatorStartProgress::StartingServices;
        ...
        let ip_echo_server = match node.sockets.ip_echo {
            None => None,
            Some(tcp_listener) => Some(put_net_utils::ip_echo_server(
                tcp_listener,
        ...
        *admin_rpc_service_post_init.write().unwrap() = Some(AdminRpcRequestMetadataPostInit {
        ...
        datapoint_info!(
            ...
            ("version", put_version::version!(), String)
        );
...
fn maybe_warp_slot(
    ...
    if let Some(warp_slot) = config.warp_slot {
        ...
        bank_forks.insert(Bank::warp_from_parent(
            ...
            &Pubkey::default(),
            warp_slot,
            put_runtime::accounts_db::CalcAccountsHashDataSource::Storages,
        ));
        bank_forks.set_root(
            warp_slot,
            accounts_background_request_sender,
...
mod tests {
    use {
        ...
        put_gossip::contact_info::{ContactInfo, LegacyContactInfo},
        put_ledger::{create_new_tmp_ledger, genesis_utils::create_genesis_config_with_leader},
        put_sdk::{genesis_config::create_genesis_config, poh_config::PohConfig},
        put_tpu_client::tpu_client::{
    ...
    fn test_backup_and_clear_blockstore() {
        ...
        use {
            put_entry::entry,
            put_ledger::{blockstore, get_tmp_ledger_path},
        };
    ...
    fn test_poh_speed() {
        ...
        let poh_config = PohConfig {
            target_tick_duration: Duration::from_millis(put_sdk::clock::MS_PER_TICK),
            // make PoH rate really fast to cause the panic condition
            hashes_per_tick: Some(100 * put_sdk::clock::DEFAULT_HASHES_PER_TICK),
            ..PohConfig::default()
        };
