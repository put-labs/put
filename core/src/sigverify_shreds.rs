use {
    ...
    put_perf::{self, deduper::Deduper, packet::PacketBatch, recycler_cache::RecyclerCache},
    put_rayon_threadlimit::get_thread_count,
    put_runtime::{bank::Bank, bank_forks::BankForks},
    put_sdk::{clock::Slot, pubkey::Pubkey},
    std::{
        collections::HashMap,
        sync::{Arc, RwLock},
...
pub(crate) fn spawn_shred_sigverify(
    ...
    let thread_pool = ThreadPoolBuilder::new()
        ...
        .thread_name(|i| format!("putSvrfyShred{i:02}"))
        .build()
        .unwrap();
...
fn verify_packets(
    ...
    put_perf::sigverify::mark_disabled(packets, &out);
}
