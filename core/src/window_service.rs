...
use {
    ...
    put_metrics::inc_new_counter_error,
    put_perf::packet::{Packet, PacketBatch},
...
impl WindowService {
    ...
    fn start_check_duplicate_thread(
        ...
        let handle_error = || {
            inc_new_counter_error!("PUT-check-duplicate-error", 1, 1);
        };
        Builder::new()
            .name("putWinCheckDup".to_string())
            .spawn(move || {
    ...
    fn start_window_insert_thread(
        ...
        let handle_error = || {
            inc_new_counter_error!("PUT-window-insert-error", 1, 1);
        };
        let thread_pool = rayon::ThreadPoolBuilder::new()
            .num_threads(get_thread_count().min(8))
            .thread_name(|i| format!("putWinInsert{i:02}"))
            .build()
            .unwrap();
        let reed_solomon_cache = ReedSolomonCache::default();
        Builder::new()
            .name("putWinInsert".to_string())
            .spawn(move || {
