...
lazy_static! {
    static ref PAR_THREAD_POOL: ThreadPool = rayon::ThreadPoolBuilder::new()
        ...
        .thread_name(|i| format!("putEntry{i:02}"))
        .build()
        .unwrap();
}
...
fn init(name: &OsStr) {
    ...
    unsafe {
        INIT_HOOK.call_once(|| {
            let path;
            let lib_name = if let Some(perf_libs_path) = put_perf::perf_libs::locate_perf_libs()
            {
                put_perf::perf_libs::append_to_ld_library_path(
...
///
/// * For TPU: `put_core::banking_stage::BankingStage::process_and_record_transactions()`
/// * For TVU: `put_core::replay_stage::ReplayStage::replay_blockstore_into_bank()`
///
...
///
/// This enforcement is done via a call to `put_runtime::accounts::Accounts::lock_accounts()`
...
mod tests {
    use {
        super::*,
        put_perf::test_tx::{test_invalid_tx, test_tx},
        put_sdk::{
            hash::{hash, Hash},
            pubkey::Pubkey,
    ...
    fn test_transaction_signing() {
        use put_sdk::signature::Signature;
        let zero = Hash::default();
