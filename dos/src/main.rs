...
//! ```bash
//! put-dos $COMMON --num-signatures 8
//! ```
//! 1.2 With valid signatures
//! ```bash
//! put-dos $COMMON --valid-signatures --num-signatures 8
//! ```
//! 2. With blockhash and payer:
//! 2.1 Single-instruction transaction
//! ```bash
//! put-dos $COMMON --valid-blockhash --transaction-type transfer --num-instructions 1
//! ```
//! 2.2 Multi-instruction transaction
//! ```bash
//! put-dos $COMMON --valid-blockhash --transaction-type transfer --num-instructions 8
//! ```
//! 2.3 Account-creation transaction
//! ```bash
//! put-dos $COMMON --valid-blockhash --transaction-type account-creation
//! ```
//!
...
use {
    ...
    log::*,
    rand::{thread_rng, Rng},
    put_bench_tps::{bench::generate_and_fund_keypairs, bench_tps_client::BenchTpsClient},
    put_client::{connection_cache::ConnectionCache, tpu_connection::TpuConnection},
    put_core::serve_repair::{RepairProtocol, RepairRequestHeader, ServeRepair},
    put_dos::cli::*,
    put_gossip::{
...
fn get_target(
    ...
    if nodes.is_empty() {
        // skip-gossip case
        target = Some((put_sdk::pubkey::new_rand(), entrypoint_addr));
    } else {
        info!("************ NODE ***********");
        for node in nodes {
            info!("{:?}", node);
...
pub mod test {
    use {
        ...
        put_gossip::contact_info::LegacyContactInfo,
        put_local_cluster::{
            cluster::Cluster,
            local_cluster::{ClusterConfig, LocalCluster},
            validator_configs::make_identical_validator_configs,
        },
        put_rpc::rpc::JsonRpcConfig,
