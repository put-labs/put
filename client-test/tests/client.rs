...
fn test_rpc_client() {
    ...
    assert_eq!(
        client.get_version().unwrap().put_core,
        put_version::semver!()
    );
    ...
    let tx = system_transaction::transfer(&alice, &bob_pubkey, put_to_lamports("20.0"), blockhash);
    ...
    assert_eq!(
        client
            ...
            .unwrap()
            .value,
        original_alice_balance - put_to_lamports("20.0")
    );
}
