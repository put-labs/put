//! The `rpc` module implements the PUT RPC interface.

use ppl_name::instruction::get_domain;
use ppl_name::state::{get_seeds_and_key, AccountType};
use put_account_decoder::parse_name::{is_known_name_id, NameAccountType};
use put_account_decoder::parse_nft::is_known_nft_token_id;
use put_account_decoder::UiAccountData;
use put_runtime::inline_name;
use put_runtime::inline_name::{
...
use put_runtime::inline_nft::{NFT_ACCOUNT_LENGTH, NFT_ACCOUNT_OWNER_OFFSET};
...
use {
    ...
    put_account_decoder::{
        parse_token::{is_known_ppl_token_id, token_amount_to_ui_amount, UiTokenAmount},
        UiAccount, UiAccountEncoding, UiDataSliceConfig, MAX_BASE58_BYTES,
    },
    put_client::connection_cache::{ConnectionCache, Protocol},
    put_entry::entry::Entry,
    put_faucet::faucet::request_airdrop_transaction,
    put_gossip::{cluster_info::ClusterInfo, contact_info::ContactInfo},
    put_ledger::{
    ...
    put_metrics::inc_new_counter_info,
    put_perf::packet::PACKET_DATA_SIZE,
    put_rpc_client_api::{
        config::*,
    ...
    put_runtime::{
        ...
        inline_ppl_token::{PPL_TOKEN_ACCOUNT_MINT_OFFSET, PPL_TOKEN_ACCOUNT_OWNER_OFFSET},
    ...
    put_send_transaction_service::{
        send_transaction_service::{SendTransactionService, TransactionInfo},
        tpu_info::NullTpuInfo,
    },
    put_stake_program,
    put_storage_bigtable::Error as StorageError,
    put_storage_hbase::Error as HBaseStorageError,
    ...
    put_vote_program::vote_state::{VoteState, MAX_LOCKOUT_HISTORY},
    std::{
        any::type_name,
        cmp::{max, min},
...
impl Default for RpcBigtableConfig {
    fn default() -> Self {
        let bigtable_instance_name = put_storage_bigtable::DEFAULT_INSTANCE_NAME.to_string();
        let bigtable_app_profile_id = put_storage_bigtable::DEFAULT_APP_PROFILE_ID.to_string();
        Self {
...
pub struct JsonRpcRequestProcessor {
    ...
    bigtable_ledger_storage: Option<put_storage_bigtable::LedgerStorage>,
    hbase_ledger_storage: Option<put_storage_hbase::LedgerStorage>,
...
impl JsonRpcRequestProcessor {
    ...
    fn bank(&self, commitment: Option<CommitmentConfig>) -> Arc<Bank> {
        ...
        r_bank_forks.get(slot).unwrap_or_else(|| {
            ...
            // a slot.
            //
            // For more information, see https://github.com/put-labs/put/issues/11078
            warn!(
    ...
    pub fn new_from_bank(
        ...
        let cluster_info = Arc::new({
            ...
            let contact_info = ContactInfo::new_localhost(
                &keypair.pubkey(),
                put_sdk::timing::timestamp(), // wallclock
            );
    ...
    pub fn get_program_accounts(
        ...
        let keyed_accounts = {
            if let Some(owner) = get_ppl_token_owner_filter(program_id, &filters) {
                self.get_filtered_ppl_token_accounts_by_owner(&bank, program_id, &owner, filters)?
            } else if let Some(mint) = get_ppl_token_mint_filter(program_id, &filters) {
                self.get_filtered_ppl_token_accounts_by_mint(&bank, program_id, &mint, filters)?
            } else {
                self.get_filtered_program_accounts(&bank, program_id, filters)?
            }
        };
        let accounts = if is_known_ppl_token_id(program_id)
    ...
    fn check_bigtable_result<T>(
        &self,
        result: &std::result::Result<T, put_storage_bigtable::Error>,
    ) -> Result<()> {
        if let Err(put_storage_bigtable::Error::BlockNotFound(slot)) = result {
    ...
    fn check_hbase_result<T>(
        &self,
        result: &std::result::Result<T, put_storage_hbase::Error>,
    ) -> Result<()> {
        if let Err(put_storage_hbase::Error::BlockNotFound(slot)) = result {
            return Err(RpcCustomError::LongTermStorageSlotSkipped { slot: *slot }.into());
        }
        Ok(())
    }
    ...
    pub fn get_confirmed_signatures_for_address(
        ...
        if self.config.enable_rpc_transaction_history {
            ...
            // https://github.com/put-labs/put/pull/10928
            let end_slot = min(
                end_slot,
    ...
    pub fn get_stake_activation(
        ...
        if bank.epoch().saturating_sub(epoch) > put_sdk::stake_history::MAX_ENTRIES as u64 {
            return Err(Error::invalid_params(format!(
        ...
        let stake_history =
            put_sdk::account::from_account::<StakeHistory, _>(&stake_history_account)
                .ok_or_else(Error::internal_error)?;
    ...
    pub fn get_token_largest_accounts(
        ...
        if !is_known_ppl_token_id(&mint_owner) {
            return Err(Error::invalid_params(
                "Invalid param: not a Token mint".to_string(),
            ));
        }
        let mut token_balances: Vec<RpcTokenAccountBalance> = self
            .get_filtered_ppl_token_accounts_by_mint(&bank, &mint_owner, mint, vec![])?
            .into_iter()
            .map(|(address, account)| {
    ...
    pub fn get_token_accounts_by_owner(
        ...
        let keyed_accounts = self.get_filtered_ppl_token_accounts_by_owner(
            &bank,
            &token_program_id,
            owner,
            filters,
    ...
    pub fn get_token_accounts_by_delegate(
        ...
        let keyed_accounts = if let Some(mint) = mint {
            self.get_filtered_ppl_token_accounts_by_mint(&bank, &token_program_id, &mint, filters)?
        } else {
    ...
    /// Get an iterator of ppl-token accounts by owner address
    fn get_filtered_ppl_token_accounts_by_owner(
        &self,
        bank: &Arc<Bank>,
        program_id: &Pubkey,
        owner_key: &Pubkey,
        ...
        filters.push(RpcFilterType::Memcmp(Memcmp::new_raw_bytes(
            PPL_TOKEN_ACCOUNT_OWNER_OFFSET,
        ...
        if self
            .config
            .account_indexes
            .contains(&AccountIndex::PplTokenOwner)
        {
            ...
            Ok(bank
                .get_filtered_indexed_accounts(
                    &IndexKey::PplTokenOwner(*owner_key),
                    |account| {
    ...
    fn get_filtered_nft_accounts_by_owner(
        ...
        if self
            .config
            .account_indexes
            .contains(&AccountIndex::PplNftOwner)
        {
            if !self.config.account_indexes.include_key(owner_key) {
                return Err(RpcCustomError::KeyExcludedFromSecondaryIndex {
                    index_key: owner_key.to_string(),
            ...
            Ok(bank
                .get_filtered_indexed_accounts(
                    &IndexKey::PplNftOwner(*owner_key),
                    |account| {
                        account.owner() == program_id
                            && filters.iter().all(|filter_type| match filter_type {
                                RpcFilterType::DataSize(size) => {
    ...
    fn get_filtered_domain_accounts_by_owner(
        ...
        if self
            .config
            .account_indexes
            .contains(&AccountIndex::PplNameOwner)
        {
            if !self.config.account_indexes.include_key(owner_key) {
                return Err(RpcCustomError::KeyExcludedFromSecondaryIndex {
                    index_key: owner_key.to_string(),
            ...
            Ok(bank
                .get_filtered_indexed_accounts(
                    &IndexKey::PplNameOwner(*owner_key),
                    |account| {
                        account.owner() == program_id
                            && filters.iter().all(|filter_type| match filter_type {
                                RpcFilterType::DataSize(size) => {
    ...
    fn get_filtered_domain_resolve_accounts_by_parent(
            ...
            .contains(&AccountIndex::PplNameOwner)
        {
            ...
            Ok(bank
                .get_filtered_indexed_accounts(
                    &IndexKey::PplNameOwner(*parent_key),
                    |account| {
                        account.owner() == program_id
                            && filters.iter().all(|filter_type| match filter_type {
                                RpcFilterType::DataSize(size) => {
    ...
    /// Get an iterator of ppl-token accounts by mint address
    fn get_filtered_ppl_token_accounts_by_mint(
        &self,
        bank: &Arc<Bank>,
        program_id: &Pubkey,
        mint_key: &Pubkey,
        ...
        #[allow(deprecated)]
        filters.push(RpcFilterType::Memcmp(Memcmp {
            offset: PPL_TOKEN_ACCOUNT_MINT_OFFSET,
        ...
        if self
            .config
            .account_indexes
            .contains(&AccountIndex::PplTokenMint)
        {
            ...
            Ok(bank
                .get_filtered_indexed_accounts(
                    &IndexKey::PplTokenMint(*mint_key),
                    |account| {
                        account.owner() == program_id
                            && filters.iter().all(|filter_type| match filter_type {
                                RpcFilterType::DataSize(size) => {
    ...
    fn get_stake_minimum_delegation(&self, config: RpcContextConfig) -> Result<RpcResponse<u128>> {
        ...
        let stake_minimum_delegation =
            put_stake_program::get_minimum_delegation(&bank.feature_set);
...
fn get_encoded_account(
    ...
    match bank.get_account(pubkey) {
        Some(account) => {
            let response = if is_known_ppl_token_id(account.owner())
                && encoding == UiAccountEncoding::JsonParsed
            {
...
/// Analyze custom filters to determine if the result will be a subset of ppl-token accounts by
/// owner.
...
fn get_ppl_token_owner_filter(program_id: &Pubkey, filters: &[RpcFilterType]) -> Option<Pubkey> {
    if !is_known_ppl_token_id(program_id) {
        return None;
    }
    ...
    for filter in filters {
        match filter {
            ...
            RpcFilterType::Memcmp(Memcmp {
                offset: PPL_TOKEN_ACCOUNT_OWNER_OFFSET,
    ...
    if data_size_filter == Some(TokenAccount::get_packed_len() as u64) {
        if let Some(incorrect_owner_len) = incorrect_owner_len {
            info!(
                "Incorrect num bytes ({:?}) provided for ppl_token_owner_filter",
                incorrect_owner_len
            );
        }
        owner_key
    } else {
        debug!("ppl_token program filters do not match by-owner index requisites");
        None
    }
}
...
/// NOTE: `optimize_filters()` should almost always be called before using this method because of
/// the strict match on `MemcmpEncodedBytes::Bytes`.
fn get_ppl_token_mint_filter(program_id: &Pubkey, filters: &[RpcFilterType]) -> Option<Pubkey> {
    if !is_known_ppl_token_id(program_id) {
        return None;
    }
    let mut data_size_filter: Option<u64> = None;
    ...
    if data_size_filter == Some(TokenAccount::get_packed_len() as u64) {
        if let Some(incorrect_mint_len) = incorrect_mint_len {
            info!(
                "Incorrect num bytes ({:?}) provided for ppl_token_mint_filter",
                incorrect_mint_len
            );
        }
        mint
    } else {
        debug!("ppl_token program filters do not match by-mint index requisites");
        None
    }
}
...
pub mod rpc_minimal {
    ...
    pub trait Minimal {
        ...
        // TODO: Refactor `put-validator wait-for-restart-window` to not require this method, so
    ...
    impl Minimal for MinimalImpl {
        ...
        fn get_version(&self, _: Self::Metadata) -> Result<RpcVersionInfo> {
            ...
            let version = put_version::Version::default();
            Ok(RpcVersionInfo {
                put_core: version.to_string(),
            ...
            Ok(meta
                ...
                .map(|leader_schedule| {
                    let mut schedule_by_identity =
                        put_ledger::leader_schedule_utils::leader_schedule_by_identity(
    ...
    impl BankData for BankDataImpl {
        ...
        fn get_block_production(
            ...
            for identity in slot_leaders {
                ...
                if slot_history.check(slot) == put_sdk::slot_history::Check::Found {
...
pub mod rpc_accounts {
    ...
    pub trait AccountsData {
        ...
        // PPL Token-specific RPC endpoints
        // See https://github.com/put-labs/put-program-library/releases/tag/token-v2.0.0 for
        // program details
...
pub mod rpc_full {
    use {
        super::*,
        put_sdk::message::{SanitizedVersionedMessage, VersionedMessage},
    };
    #[rpc]
    pub trait Full {
        type Metadata;
...
pub fn create_test_transaction_entries(
    ...
    let success_tx = put_sdk::system_transaction::transfer(
        mint_keypair,
        &keypair1.pubkey(),
        rent_exempt_amount,
        blockhash,
    ...
    let entry_1 = put_entry::entry::next_entry(&blockhash, 1, vec![success_tx]);
    // Failed transaction, InstructionError
    let ix_error_tx = put_sdk::system_transaction::transfer(
        keypair2,
        &keypair3.pubkey(),
    ...
    let entry_2 = put_entry::entry::next_entry(&entry_1.hash, 1, vec![ix_error_tx]);
...
pub fn populate_blockstore_for_tests(
    ...
    let shreds = put_ledger::blockstore::entries_to_test_shreds(
        &entries,
        slot,
        parent_slot,
        true,
    ...
    assert_eq!(
        put_ledger::blockstore_processor::process_entries_for_tests(
            &bank,
            entries,
            true,
            Some(
                &put_ledger::blockstore_processor::TransactionStatusSender {
...
pub mod tests {
    use {
        ...
        ppl_token::{
            put_program::{program_option::COption, pubkey::Pubkey as PplTokenPubkey},
            state::{AccountState as TokenAccountState, Mint},
        },
        serde::de::DeserializeOwned,
        put_address_lookup_table_program::state::{AddressLookupTable, LookupTableMeta},
        put_entry::entry::next_versioned_entry,
        put_gossip::socketaddr,
        put_ledger::{
    ...
    fn ppl_token_id() -> Pubkey {
        put_account_decoder::parse_token::ppl_token_ids()[0]
    }
    ...
    impl RpcHandler {
        ...
        fn store_address_lookup_table(&self) -> Pubkey {
            ...
            let address_table_account = {
                ...
                AccountSharedData::create(
                    ...
                    address_table_data,
                    put_address_lookup_table_program::id(),
                    false,
                    0,
                )
            };
        ...
        fn store_vote_account(&self, vote_pubkey: &Pubkey, vote_state: VoteState) {
            ...
            let mut vote_account =
                AccountSharedData::new(balance, space, &put_vote_program::id());
    ...
    fn test_rpc_request_processor_new() {
        let bob_pubkey = put_sdk::pubkey::new_rand();
    ...
    fn test_token_rpcs() {
        ...
        let mint = PplTokenPubkey::from([2; 32]);
        let owner = PplTokenPubkey::from([3; 32]);
        let delegate = PplTokenPubkey::from([4; 32]);
        ...
        let token_account_pubkey = put_sdk::pubkey::new_rand();
        ...
        let other_token_account_pubkey = put_sdk::pubkey::new_rand();
        bank.store_account(&other_token_account_pubkey, &token_account);

        // Add another token account with the same owner and delegate but different mint
        let mut account_data = vec![0; TokenAccount::get_packed_len()];
        let new_mint = PplTokenPubkey::from([5; 32]);
        let token_account = TokenAccount {
            mint: new_mint,
            owner,
            delegate: COption::Some(delegate),
        ...
        let token_with_different_mint_pubkey = put_sdk::pubkey::new_rand();
        ...
        let token_with_smaller_balance = put_sdk::pubkey::new_rand();
    ...
    #[test]
    fn test_get_ppl_token_owner_filter() {
        let owner = Pubkey::new_unique();
        assert_eq!(
            get_ppl_token_owner_filter(
                &Pubkey::from_str("PutToken11111111111111111111111111111111111").unwrap(),
                &[
        ...
        assert!(get_ppl_token_owner_filter(
            &Pubkey::from_str("PutToken11111111111111111111111111111111111").unwrap(),
            &[
    ...
    fn test_decode_and_deserialize_too_large_payloads_fail() {
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx58, TransactionBinaryEncoding::Base58)
                .unwrap_err(),
            Error::invalid_params(format!(
                "base58 encoded put_sdk::transaction::Transaction too large: {tx58_len} bytes (max: encoded/raw {MAX_BASE58_SIZE}/{PACKET_DATA_SIZE})",
            )
        ));
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx64, TransactionBinaryEncoding::Base64)
                .unwrap_err(),
            Error::invalid_params(format!(
                "base64 encoded put_sdk::transaction::Transaction too large: {tx64_len} bytes (max: encoded/raw {MAX_BASE64_SIZE}/{PACKET_DATA_SIZE})",
            )
        ));
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx58, TransactionBinaryEncoding::Base58)
                .unwrap_err(),
            Error::invalid_params(format!(
                "decoded put_sdk::transaction::Transaction too large: {too_big} bytes (max: {PACKET_DATA_SIZE} bytes)"
            ))
        );
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx64.clone(), TransactionBinaryEncoding::Base64)
                .unwrap_err(),
            Error::invalid_params(
                "failed to deserialize put_sdk::transaction::Transaction: invalid value: \
