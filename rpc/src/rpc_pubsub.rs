...
use {
    ...
    put_account_decoder::{UiAccount, UiAccountEncoding},
    put_rpc_client_api::{
        config::{
    ...
    put_transaction_status::UiTransactionEncoding,
    std::{str::FromStr, sync::Arc},
};

// We have to keep both of the following traits to not break backwards compatibility.
// `RpcPutPubSubInternal` is actually used by the current PubSub API implementation.
// `RpcPutPubSub` and the corresponding `gen_client` module are preserved
...
pub use internal::RpcPutPubSubInternal;
...
mod internal {
    ...
    #[rpc]
    pub trait RpcPutPubSubInternal {
        // Get notification every time account data is changed
        // Accepts pubkey parameter as base-58 encoded string
...
impl RpcPutPubSubInternal for RpcPutPubSubImpl {
    fn account_subscribe(
        &self,
        pubkey_str: String,
        config: Option<RpcAccountInfoConfig>,
...
mod tests {
    use {
        ...
        serial_test::serial,
        put_account_decoder::{parse_account_data::parse_account_data, UiAccountEncoding},
        put_rpc_client_api::response::{
        ...
        put_vote_program::vote_state::Vote,
        std::{
            sync::{
    ...
    fn test_account_subscribe() {
        ...
        let new_stake_authority = put_sdk::pubkey::new_rand();
    ...
    fn test_get_version() {
        ...
        let expected_version = put_version::Version::default();
