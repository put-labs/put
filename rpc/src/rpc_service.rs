//! The `rpc_service` module implements the PUT JSON RPC service.
...
use {
    ...
    regex::Regex,
    put_client::connection_cache::ConnectionCache,
    ...
    put_perf::thread::renice_this_thread,
    put_poh::poh_recorder::PohRecorder,
    put_runtime::{
    ...
    put_sdk::{
        ...
        native_token::lamports_to_put, pubkey::Pubkey,
    },
    put_send_transaction_service::send_transaction_service::{self, SendTransactionService},
    ...
    match path {
        "/v0/circulating-supply" => {
            ...
            let non_circulating_supply =
                put_runtime::non_circulating_supply::calculate_non_circulating_supply(&bank)
                    .expect("Scan should not error on root banks")
                    .lamports;
            Some(format!(
                "{}",
                lamports_to_put(total_supply - non_circulating_supply)
            ))
        }
        "/v0/total-supply" => {
            let bank = bank_forks.read().unwrap().root_bank();
            ...
            Some(format!("{}", lamports_to_put(total_supply - 14703040)))
        }
        _ => None,
    }
}
...
impl JsonRpcService {
    ...
    pub fn new(
        ...
        let (bigtable_ledger_storage, _bigtable_ledger_upload_service) =
            ...
            {
                let bigtable_config = put_storage_bigtable::LedgerStorageConfig {
                ...
                runtime
                    .block_on(put_storage_bigtable::LedgerStorage::new_with_config(
                        bigtable_config,
                    ))
        ...
        if config.rpc_hbase_config.is_some() {
            put_storage_hbase::LedgerStorage::new(
...
mod tests {
    use {
        ...
        put_rpc_client_api::config::RpcContextConfig,
    ...
    fn test_rpc_new() {
        ...
        let rpc_addr = SocketAddr::new(
            ip_addr,
            put_net_utils::find_available_port_in_range(ip_addr, (10000, 65535)).unwrap(),
        );
        ...
        assert_eq!(thread.name().unwrap(), "putJsonRpcSvc");
