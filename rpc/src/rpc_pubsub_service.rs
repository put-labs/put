...
use {
    crate::{
        rpc_pubsub::{RpcPutPubSubImpl, RpcPutPubSubInternal},
...
pub fn test_connection(
    subscriptions: &Arc<RpcSubscriptions>,
) -> (RpcPutPubSubImpl, TestBroadcastReceiver) {
    let current_subscriptions = Arc::new(DashMap::new());

    let rpc_impl = RpcPutPubSubImpl::new(
        PubSubConfig {
...
mod tests {
    ...
    fn test_pubsub_new() {
        ...
        assert_eq!(thread.name().unwrap(), "putRpcPubSub");
    }
}
