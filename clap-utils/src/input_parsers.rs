...
pub fn lamports_of_put(matches: &ArgMatches<'_>, name: &str) -> Option<u128> {
    value_of(matches, name).and_then(|p:String| Some(put_to_lamports(&p)))
}
...
mod tests {
    ...
    #[test]
    fn test_lamports_of_put() {
        let matches = app().get_matches_from(vec!["test", "--single", "50"]);
        assert_eq!(lamports_of_put(&matches, "single"), Some(50_000_000_000));
        assert_eq!(lamports_of_put(&matches, "multiple"), None);
        let matches = app().get_matches_from(vec!["test", "--single", "1.5"]);
        assert_eq!(lamports_of_put(&matches, "single"), Some(1_500_000_000));
        assert_eq!(lamports_of_put(&matches, "multiple"), None);
        let matches = app().get_matches_from(vec!["test", "--single", "0.03"]);
        assert_eq!(lamports_of_put(&matches, "single"), Some(30_000_000));
    }
}
