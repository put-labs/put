...
//! standard signing sources, from the command line, as in the PUT CLI.
//!
...
//! sources supported by the PUT CLI. Many other functions here are
...
impl DefaultSigner {
    ...
    /// use put_clap_utils::keypair::DefaultSigner;
    /// use put_clap_utils::offline::OfflineArgs;
    ///
    ...
    /// ```no_run
    /// use clap::{App, Arg, value_t_or_exit};
    /// use put_clap_utils::keypair::{DefaultSigner, signer_from_path};
    /// use put_clap_utils::offline::OfflineArgs;
    ...
    /// ```no_run
    /// use clap::{App, Arg, value_t_or_exit};
    /// use put_clap_utils::keypair::{SignerFromPathConfig, DefaultSigner};
    /// use put_clap_utils::offline::OfflineArgs;
    ///
    /// let clap_app = App::new("my-program")
    ///     // The argument we'll parse as a signer "path"
...
/// ```no_run
/// use clap::{App, Arg, value_t_or_exit};
/// use put_clap_utils::keypair::signer_from_path;
/// use put_clap_utils::offline::OfflineArgs;
///
/// let clap_app = App::new("my-program")
///     // The argument we'll parse as a signer "path"
...
/// ```no_run
/// use clap::{App, Arg, value_t_or_exit};
/// use put_clap_utils::keypair::{signer_from_path_with_config, SignerFromPathConfig};
/// use put_clap_utils::offline::OfflineArgs;
///
/// let clap_app = App::new("my-program")
///     // The argument we'll parse as a signer "path"
    ...
    match kind {
        ...
        SignerSourceKind::Filepath(path) => match read_keypair_file(&path) {
            Err(e) => Err(std::io::Error::new(
                std::io::ErrorKind::Other,
                format!("could not read keypair file \"{}\". Run \"put-keygen new\" to create a keypair file: {}", path, e),
            )
            .into()),
        ...
        SignerSourceKind::Filepath(path) => match read_keypair_file(&path) {
            Err(e) => Err(std::io::Error::new(
                ...
                format!(
                    ...
                    Run \"put-keygen new\" to create a keypair file: {}",
                    path, e
                ),
            )
            .into()),
...
/// ```no_run
/// use clap::{App, Arg, value_t_or_exit};
/// use put_clap_utils::keypair::keypair_from_path;
///
/// let clap_app = App::new("my-program")
///     // The argument we'll parse as a signer "path"
///     .arg(Arg::with_name("keypair")
