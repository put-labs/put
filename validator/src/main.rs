...
use {
    ...
    put_clap_utils::input_parsers::{keypair_of, keypairs_of, pubkey_of, value_of},
    put_core::{
    ...
    put_gossip::{cluster_info::Node, legacy_contact_info::LegacyContactInfo as ContactInfo},
    put_ledger::blockstore_options::{
        BlockstoreCompressionType, BlockstoreRecoveryMode, LedgerColumnOptions, ShredStorageType,
    },
    put_perf::recycler::enable_recycler_warming,
    put_poh::poh_service,
    put_rpc::{
    ...
    put_rpc_client_api::config::RpcLeaderScheduleConfig,
    put_runtime::{
        accounts_db::{
    ...
    put_send_transaction_service::send_transaction_service,
    put_streamer::socket::SocketAddrSpace,
    put_tpu_client::tpu_client::DEFAULT_TPU_ENABLE_UDP,
    put_validator::{
        admin_rpc_service,
...
fn get_cluster_shred_version(entrypoints: &[SocketAddr]) -> Option<u16> {
    ...
    for entrypoint in entrypoints {
        match put_net_utils::get_cluster_shred_version(entrypoint) {
...
pub fn main() {
    ...
    let put_version = put_version::version!();
    let cli_app = app(put_version, &default_args);
        ...
        ("set-public-address", Some(subcommand_matches)) => {
            let parse_arg_addr = |arg_name: &str, arg_long: &str| -> Option<SocketAddr> {
                subcommand_matches.value_of(arg_name).map(|host_port| {
                        put_net_utils::parse_host_port(host_port).unwrap_or_else(|err| {
    ...
    let logfile = {
        let logfile = matches
            .value_of("logfile")
            .map(|s| s.into())
            .unwrap_or_else(|| format!("put-validator-{}.log", identity_keypair.pubkey()));
    ...
    info!("{} {}", crate_name!(), put_version::version!());
    info!("Starting validator with: {:#?}", std::env::args_os());

    let cuda = matches.is_present("cuda");
    if cuda {
        put_perf::perf_libs::init_cuda();
        enable_recycler_warming();
    }

    put_core::validator::report_target_features();
    ...
    let bind_address = put_net_utils::parse_host(matches.value_of("bind_address").unwrap())
        .expect("invalid bind_address");
    let rpc_bind_address = if matches.is_present("rpc_bind_address") {
        put_net_utils::parse_host(matches.value_of("rpc_bind_address").unwrap())
            .expect("invalid rpc_bind_address")
    } else if private_rpc {
        put_net_utils::parse_host("127.0.0.1").unwrap()
    } else {
        bind_address
    };
    ...
    let entrypoint_addrs = values_t!(matches, "entrypoint", String)
        ...
        .into_iter()
        .map(|entrypoint| {
            put_net_utils::parse_host_port(&entrypoint).unwrap_or_else(|e| {
    ...
    let tower_storage: Arc<dyn put_core::tower_storage::TowerStorage> =
        ...
        || matches.is_present("enable_hbase_ledger_upload")
    {
        Some(RpcHBaseConfig {
            ...
            hbase_rpc_url: matches.value_of("hbase_rpc_url").map(|address| {
                put_net_utils::parse_host_port(address).expect("failed to parse faucet address").to_string()
            }).unwrap(),
        })
    } else {
        None
    ...
    let mut validator_config = ValidatorConfig {
        ...
        rpc_config: JsonRpcConfig {
            ...
            faucet_addr: matches.value_of("rpc_faucet_addr").map(|address| {
                put_net_utils::parse_host_port(address).expect("failed to parse faucet address")
            }),
            full_api,
    ...
    let dynamic_port_range =
        put_net_utils::parse_port_range(matches.value_of("dynamic_port_range").unwrap())
    ...
    let public_rpc_addr = matches.value_of("public_rpc_addr").map(|addr| {
        put_net_utils::parse_host_port(addr).unwrap_or_else(|e| {
    ...
    if !matches.is_present("no_os_network_limits_test") {
        if SystemMonitorService::check_os_network_limits() {
            ...
            eprintln!("OS network limit test failed. See: https://docs.put.com/running-validator/validator-start#system-tuning");
            exit(1);
        }
    }
    ...
    let gossip_host: IpAddr = matches
        ...
        .map(|gossip_host| {
            put_net_utils::parse_host(gossip_host).unwrap_or_else(|err| {
        ...
        .unwrap_or_else(|| {
            if !entrypoint_addrs.is_empty() {
                ...
                let gossip_host = order.into_iter().find_map(|i| {
                    ...
                    put_net_utils::get_public_ip_addr(entrypoint_addr).map_or_else(
                        |err| {
                            eprintln!(
    ...
    let gossip_addr = SocketAddr::new(
        ...
        value_t!(matches, "gossip_port", u16).unwrap_or_else(|_| {
            put_net_utils::find_available_port_in_range(bind_address, (0, 1)).unwrap_or_else(
                |err| {
    ...
    let public_tpu_addr = matches.value_of("public_tpu_addr").map(|public_tpu_addr| {
        put_net_utils::parse_host_port(public_tpu_addr).unwrap_or_else(|err| {
    ...
    let public_tpu_forwards_addr =
        matches
            ...
            .map(|public_tpu_forwards_addr| {
                put_net_utils::parse_host_port(public_tpu_forwards_addr).unwrap_or_else(|err| {
    ...
    put_metrics::set_host_id(identity_keypair.pubkey().to_string());
    put_metrics::set_panic_hook("validator", Some(String::from(put_version)));
    put_entry::entry::init_poh();
...
fn process_account_indexes(matches: &ArgMatches) -> AccountSecondaryIndexes {
    let account_indexes: HashSet<AccountIndex> = matches
        ...
        .map(|value| match value {
            ...
            "ppl-token-mint" => AccountIndex::PplTokenMint,
            "ppl-token-owner" => AccountIndex::PplTokenOwner,
            "ppl-nft-owner" => AccountIndex::PplNftOwner,
            "ppl-name-owner" => AccountIndex::PplNameOwner,
            _ => unreachable!(),
        })
        .collect();
