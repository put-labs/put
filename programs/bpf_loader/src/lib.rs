...
macro_rules! create_vm {
    ($vm:ident, $program:expr, $regions:expr, $orig_account_lengths:expr, $invoke_context:expr $(,)?) => {
        ...
        let heap_size = invoke_context
            .get_compute_budget()
            .heap_size
            .unwrap_or(put_sdk::entrypoint::HEAP_LENGTH);
        let round_up_heap_size = invoke_context
            .feature_set
            .is_active(&put_sdk::feature_set::round_up_heap_size::id());
        ...
        let $vm = heap_cost_result.and_then(|_| {
            let mut stack = put_rbpf::aligned_memory::AlignedMemory::<
                { put_rbpf::ebpf::HOST_ALIGN },
            >::zero_filled(stack_size);
            let mut heap = put_rbpf::aligned_memory::AlignedMemory::<
                { put_rbpf::ebpf::HOST_ALIGN },
...
macro_rules! mock_create_vm {
    ($vm:ident, $additional_regions:expr, $orig_account_lengths:expr, $invoke_context:expr $(,)?) => {
        let loader = std::sync::Arc::new(BuiltinProgram::new_loader(
            put_rbpf::vm::Config::default(),
        ));
        let function_registry = put_rbpf::vm::FunctionRegistry::default();
        let executable = put_rbpf::elf::Executable::<
            put_rbpf::verifier::TautologyVerifier,
            InvokeContext,
        >::from_text_bytes(
            &[0x95, 0, 0, 0, 0, 0, 0, 0],
            loader,
        ...
        .unwrap();
        let verified_executable = put_rbpf::elf::Executable::verified(executable).unwrap();
        $crate::create_vm!(
            $vm,
            &verified_executable,
            $additional_regions,
    ...
    match limited_deserialize(instruction_data)? {
        ...
        UpgradeableLoaderInstruction::DeployWithMaxDataLen { max_data_len } => {
            ...
            let signers = [[new_program_id.as_ref(), &[bump_seed]]]
                ...
                .collect::<Result<Vec<Pubkey>, put_sdk::pubkey::PubkeyError>>()?;
...
pub mod test_utils {
    use {
        super::*, put_program_runtime::loaded_programs::DELAY_VISIBILITY_SLOT_OFFSET,
        put_sdk::account::ReadableAccount,
    };
