...
use {
    ...
    log::*,
    put_program::vote::{instruction::VoteInstruction, program::id, state::VoteAuthorize},
    put_program_runtime::{
...
mod tests {
    ...
    fn create_test_account() -> (Pubkey, AccountSharedData) {
        ...
        (
            vote_pubkey,
            vote_state::create_account(&vote_pubkey, &put_sdk::pubkey::new_rand(), 0, balance),
        )
    }

    fn create_test_account_with_authorized() -> (Pubkey, Pubkey, Pubkey, AccountSharedData) {
        let vote_pubkey = put_sdk::pubkey::new_rand();
        let authorized_voter = put_sdk::pubkey::new_rand();
        let authorized_withdrawer = put_sdk::pubkey::new_rand();
    ...
    fn test_authorize_voter() {
        let (vote_pubkey, vote_account) = create_test_account();
        let authorized_voter_pubkey = put_sdk::pubkey::new_rand();
        let clock = Clock {
            epoch: 1,
    ...
    fn test_authorize_withdrawer() {
        let (vote_pubkey, vote_account) = create_test_account();
        let authorized_withdrawer_pubkey = put_sdk::pubkey::new_rand();
        let instruction_data = serialize(&VoteInstruction::Authorize(
