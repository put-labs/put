use {
    ...
    put_sdk::derivation_path::DerivationPath,
    std::{fmt, sync::Arc},
};
#[cfg(feature = "hidapi")]
use {
    ...
    put_sdk::{pubkey::Pubkey, signature::Signature},
...
impl RemoteWallet<hidapi::DeviceInfo> for LedgerWallet {
    ...
    fn sign_offchain_message(
        ...
        if message.len()
            > put_sdk::offchain_message::v0::OffchainMessage::MAX_LEN_LEDGER
                + put_sdk::offchain_message::v0::OffchainMessage::HEADER_LEN
        {
            return Err(RemoteWalletError::InvalidInput(
