use {
    ...
    put_config_program::{config_instruction, get_config_data, ConfigState},
...
fn check_env_path_for_bin_dir(config: &Config) {
    ...
    if !found {
        println!(
            "\nPlease update your PATH environment variable to include the put programs:\n    PATH=\"{}:$PATH\"\n",
...
pub fn init(
    ...
    let config = {
        // Write new config file only if different, so that running |put-install init|
...
fn github_release_download_url(release_semver: &str) -> String {
    format!(
        "https://github.com/put-labs/put/releases/download/v{}/put-release-{}.tar.bz2",
        release_semver,
...
fn release_channel_download_url(release_channel: &str) -> String {
    format!(
        "https://release.put.com/{}/put-release-{}.tar.bz2",
        release_channel,
        crate::build_env::TARGET
    )
}
...
fn release_channel_version_url(release_channel: &str) -> String {
    format!(
        "https://release.put.com/{}/put-release-{}.yml",
        release_channel,
        crate::build_env::TARGET
    )
}
...
pub fn info(config_file: &str, local_info_only: bool, eval: bool) -> Result<(), String> {
    ...
    if eval {
        ...
        config
            ...
            .and_then(|channel| {
                println!("PUT_INSTALL_ACTIVE_CHANNEL={}", channel,);
                Option::<String>::None
            });
        return Ok(());
    }
...
fn check_for_newer_github_release(
    ...
    while page == 1 || releases.len() == PER_PAGE {
        let url = reqwest::Url::parse_with_params(
            "https://api.github.com/repos/put-labs/put/releases",
            &[
        ...
        match explicit_release {
            ...
            ExplicitRelease::Channel(release_channel) => {
                ...
                let current_release_version_yml =
                    release_dir.join("put-release").join("version.yml");
