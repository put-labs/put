...
use {
    ...
    put_zk_token_sdk::encryption::{auth_encryption::AeKey, elgamal::ElGamalKeypair},
    std::{
        cell::RefCell,
        convert::TryFrom,
        error,
...
impl DefaultSigner {
    ...
    /// use put_clap_v3_utils::keypair::DefaultSigner;
    /// use put_clap_v3_utils::offline::OfflineArgs;
    ///
            ...
            parse_signer_source(&self.path)
                ...
                .map_err(|_| {
                    std::io::Error::new(
                        ...
                        format!(
                        "No default signer found, run \"put-keygen new -o {}\" to create a new one",
                        self.path
                    ),
                    )
                })?;
    ...
    /// ```no_run
    /// use clap::{Arg, Command};
    /// use put_clap_v3_utils::keypair::{DefaultSigner, signer_from_path};
    /// use put_clap_v3_utils::offline::OfflineArgs;
    /// use put_sdk::signer::Signer;
    ///
    /// let clap_app = Command::new("my-program")
    ///     // The argument we'll parse as a signer "path"
    ///     .arg(Arg::new("keypair")
    ...
    /// ```no_run
    /// use clap::{Arg, Command};
    /// use put_clap_v3_utils::keypair::{SignerFromPathConfig, DefaultSigner};
    /// use put_clap_v3_utils::offline::OfflineArgs;
    ///
    /// let clap_app = Command::new("my-program")
    ///     // The argument we'll parse as a signer "path"
...
/// ```no_run
/// use clap::{Arg, Command};
/// use put_clap_v3_utils::keypair::signer_from_path;
/// use put_clap_v3_utils::offline::OfflineArgs;
///
/// let clap_app = Command::new("my-program")
///     // The argument we'll parse as a signer "path"
...
/// ```no_run
/// use clap::{Arg, Command};
/// use put_clap_v3_utils::keypair::{signer_from_path_with_config, SignerFromPathConfig};
/// use put_clap_v3_utils::offline::OfflineArgs;
///
/// let clap_app = Command::new("my-program")
///     // The argument we'll parse as a signer "path"
    ...
    match kind {
        ...
        SignerSourceKind::Filepath(path) => match read_keypair_file(&path) {
            Err(e) => Err(std::io::Error::new(
                std::io::ErrorKind::Other,
                format!("could not read keypair file \"{path}\". Run \"put-keygen new\" to create a keypair file: {e}"),
            )
            .into()),
...
/// ```no_run
/// use clap::{Arg, Command};
/// use put_clap_v3_utils::keypair::pubkey_from_path;
///
/// let clap_app = Command::new("my-program")
///     // The argument we'll parse as a signer "path"
///     .arg(Arg::new("keypair")
        ...
        SignerSourceKind::Filepath(path) => match read_keypair_file(&path) {
            Err(e) => Err(std::io::Error::new(
                ...
                format!(
                    ...
                    Run \"put-keygen new\" to create a keypair file: {e}"
                ),
            )
            .into()),
...
/// ```no_run
/// use clap::{Arg, Command};
/// use put_clap_v3_utils::keypair::keypair_from_path;
///
/// let clap_app = Command::new("my-program")
///     // The argument we'll parse as a signer "path"
///     .arg(Arg::new("keypair")
...
/// ```no_run`
/// use clap::{Arg, Command};
/// use put_clap_v3_utils::keypair::elgamal_keypair_from_path;
///
/// let clap_app = Command::new("my-program")
///     // The argument we'll parse as a signer "path"
...
/// ```no_run`
/// use clap::{Arg, Command};
/// use put_clap_v3_utils::keypair::ae_key_from_path;
///
/// let clap_app = Command::new("my-program")
///     // The argument we'll parse as a signer "path"
///     .arg(Arg::new("ae-key")
...
mod tests {
    use {
        ...
        clap::{Arg, Command},
        put_remote_wallet::{locator::Manufacturer, remote_wallet::initialize_wallet_manager},
        put_sdk::{signer::keypair::write_keypair_file, system_instruction},
